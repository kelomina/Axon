...
import src.loop167_phase_b.lease_v9 as lease_v9
from src.loop167_phase_b.contracts import PhaseBContractError, canonical_json_bytes
from src.loop167_phase_b.execution_authorization_v9 import VerifiedExecutionAuthorizationV9
from src.loop167_phase_b.execution_contract_v9 import (
    EXPECTED_LEASE,
    FIXED_OUTPUT_CATALOG,
    RUN_AUTHORIZATION_RELATIVE_PATH,
)
from src.loop167_phase_b.lease_v9 import (
    ExecutionLeaseV8Error,
    build_execution_lease_payload_v9,
    consume_execution_lease_v9,
    verify_consumed_execution_lease_v9,
)
from src.loop167_phase_b.supervisor_v9 import ValidatedLaunchReceiptV9
...
def _authorization(root: Path, *, digest: str = "a" * 64) -> VerifiedExecutionAuthorizationV9:
...
def _install_verification_mocks(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ValidatedLaunchReceiptV9(
    ...
    monkeypatch.setattr(lease_v9, "validate_execution_authorization_v9", lambda *_args, **_kwargs: authorization)
    real_sha256_file = lease_v9.sha256_file
    monkeypatch.setattr(
        lease_v9,
        "sha256_file",
        lambda path: (
...
def test_v7_lease_is_one_shot_and_binds_the_validated_launch_digest(
    ...
    consumed = consume_execution_lease_v9(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)
    verified = verify_consumed_execution_lease_v9(
        root,
        authorization,
        launch_id=LAUNCH_ID,
        now_utc=NOW,
    ...
    with pytest.raises(ExecutionLeaseV8Error, match="already exists"):
        consume_execution_lease_v9(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)
...
def test_v7_lease_rejects_a_child_attestation_with_a_different_exact_launch_digest(
    ...
    with pytest.raises(ExecutionLeaseV8Error, match="launch receipt digest drifted"):
        build_execution_lease_payload_v9(authorization, launch_id=LAUNCH_ID, consumed_at_utc=NOW)
...
def test_v7_lease_revalidates_authorization_after_consumption(
    ...
    monkeypatch.setattr(lease_v9, "validate_execution_authorization_v9", lambda *_args, **_kwargs: drifted)

    with pytest.raises(ExecutionLeaseV8Error, match="authorization drifted after lease"):
        verify_consumed_execution_lease_v9(root, authorization, launch_id=LAUNCH_ID, now_utc=NOW)
...
def test_v7_lease_rejects_a_marker_replaced_after_its_canonical_read(
    ...
    _install_verification_mocks(monkeypatch, authorization)
    consume_execution_lease_v9(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)
    original_builder = lease_v9.build_execution_lease_payload_v9
    ...
    monkeypatch.setattr(lease_v9, "build_execution_lease_payload_v9", replace_marker_after_expected_payload)
