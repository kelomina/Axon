...
import src.loop167_phase_b.execution_authorization_v12 as execution_authorization_v12
...
from src.loop167_phase_b.resource_guard_v12 import RESOURCE_GUARD_SCHEMA
...
def _authorization_fixture(
    ...
    _write_json(
        ...
        RUNTIME_LOCK_RELATIVE_PATH,
        {
            "schema": "axon_loop167_phase_b_runtime_lock_v12",
            "loop_id": LOOP_ID,
    ...
    monkeypatch.setattr(execution_authorization_v12, "verify_execution_contract_v12", verify_contract)
    monkeypatch.setattr(execution_authorization_v12, "verify_resource_guard_v12", verify_guard)
    authorization_payload = execution_authorization_v12.build_execution_authorization_payload_v12(
        root,
...
def _validate_prelaunch(root: Path, context: dict[str, object]) -> object:
    return execution_authorization_v12.validate_execution_authorization_v12(
        root,
...
def test_v7_authorization_prelaunch_and_attested_child_surfaces(
    ...
    with pytest.raises(PhaseBContractError, match="attested child requires"):
        execution_authorization_v12.validate_execution_authorization_v12(
            root,
            context["authorization_path"],
            now_utc=NOW,
    ...
    monkeypatch.setattr(execution_authorization_v12, "validate_launch_receipt_v12", validate_launch)
    monkeypatch.setattr(execution_authorization_v12, "verify_child_job_attestation_v12", verify_attestation)
    verified = execution_authorization_v12.validate_execution_authorization_v12(
        root,
        context["authorization_path"],
        now_utc=NOW,
        phase="attested_child",
