...
import src.loop167_phase_b.supervisor_v8 as supervisor_v8
...
def _config(root: Path) -> SupervisorConfigV8:
    ...
    output = root / "reports" / "v8"
    return SupervisorConfigV8(
        project_root=root,
        mode="preflight",
...
def test_supervisor_persists_receipt_before_resume(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("src.loop167_phase_b.supervisor_v8._fsync_parent_directory", lambda _parent: None)
    job = FakeJob(config.launch_receipt)

    result = run_supervised_v8(
        config,
    ...
    assert validated.canonical_sha256 == supervisor_v8.hashlib.sha256(config.launch_receipt.read_bytes()).hexdigest()
...
def test_supervisor_rejects_controller_paths_other_than_the_sealed_relative_path(
    ...
    with pytest.raises(SupervisorV8Error, match="sealed relative path"):
        run_supervised_v8(invalid, job_factory=lambda **_kwargs: pytest.fail("must not spawn"))
...
def test_supervisor_does_not_resume_when_launch_receipt_cannot_be_written(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SupervisorV8Error, match="output already exists"):
        run_supervised_v8(
            config,
            job_factory=lambda **_kwargs: job,
            supervisor_identity_provider=_supervisor_identity,
...
def test_launch_receipt_rejects_static_binding_tampering(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SupervisorV8Error, match="static bindings drifted"):
        validate_launch_receipt_v8(
            tmp_path,
            config.launch_receipt,
            mode="preflight",
...
def test_supervisor_does_not_resume_when_parent_sync_fails(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(
        "src.loop167_phase_b.supervisor_v8._fsync_parent_directory",
        lambda _parent: (_ for _ in ()).throw(SupervisorV8Error("sync failed")),
    )
    job = FakeJob(config.launch_receipt)

    with pytest.raises(SupervisorV8Error, match="sync failed"):
        run_supervised_v8(
            config,
            job_factory=lambda **_kwargs: job,
            supervisor_identity_provider=_supervisor_identity,
...
def test_windows_parent_sync_returns_without_posix_fallback(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(supervisor_v8.os, "name", "nt")
    monkeypatch.setattr(ctypes, "WinDLL", lambda *_args, **_kwargs: Kernel32(), raising=False)
    monkeypatch.setattr(supervisor_v8.os, "open", lambda *_args, **_kwargs: pytest.fail("POSIX fallback"))

    supervisor_v8._fsync_parent_directory(tmp_path)
...
def test_launch_receipt_rejects_a_change_after_its_exact_bytes_are_read(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(supervisor_v8, "sha256_file", lambda _path: "0" * 64)

    with pytest.raises(SupervisorV8Error, match="changed during validation"):
        validate_launch_receipt_v8(
            tmp_path,
            config.launch_receipt,
            mode="preflight",
...
def test_supervisor_terminates_and_closes_the_job_after_child_timeout(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SupervisorV8Error, match="exceeded"):
        run_supervised_v8(
            config,
            job_factory=lambda **_kwargs: job,
            supervisor_identity_provider=_supervisor_identity,
...
def test_supervisor_records_nonzero_exit_and_closes_the_job(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SupervisorV8Error, match="nonzero: 7"):
        run_supervised_v8(
            config,
            job_factory=lambda **_kwargs: job,
            supervisor_identity_provider=_supervisor_identity,
...
def test_supervisor_terminates_a_suspended_child_when_pre_resume_audit_fails(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(SupervisorV8Error, match="assignment audit drifted"):
        run_supervised_v8(
            config,
            job_factory=lambda **_kwargs: job,
            supervisor_identity_provider=_supervisor_identity,
