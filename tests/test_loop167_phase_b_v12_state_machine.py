...
import src.loop167_phase_b.preflight_v12 as preflight_v12
...
from src.loop167_phase_b.preflight_v12 import (
    EXPECTED_BLOCKERS,
...
def _preflight_fixture(root: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    _write(root, PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH, canonical_json_bytes({"parent": "synthetic"}))
    ...
    source_payload = {
        ...
        "loop_id": LOOP_ID,
        "scope": V12_SCOPE,
        "parent_v10_prelease_attestation": _binding(root, PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH),
    ...
    monkeypatch.setattr(preflight_v12, "verify_parent_v10_prelease_attestation_v12", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        preflight_v12,
...
def test_v7_prelaunch_attested_and_leased_pre_raw_surfaces_transition_without_raw_access(
    ...
    assert_output_catalog_is_fresh_v12(root)
    launch = _output_path(root, "supervisor_launch_receipt")
    ...
    assert_attested_child_prelease_surface_v12(root)
    with pytest.raises(PhaseBContractError, match="Project path is missing"):
        assert_leased_child_pre_raw_surface_v12(root)
...
def test_v7_post_lease_guard_rejects_data_plane_source_drift_before_any_raw_open(
    ...
    initial = preflight_v12.validate_static_preflight_v12(
        root,
    ...
    with pytest.raises(PhaseBContractError, match="hash mismatch"):
        preflight_v12.validate_static_preflight_v12(
            root,
            source_closure_binding=bindings["source_closure"],
            controller_binding=bindings["controller"],
            supervisor_binding=bindings["supervisor"],
