...
from src.loop167_phase_b import loop166_v6_bridge
from src.loop167_phase_b.contracts import PhaseBContractError
from src.loop167_phase_b.loop166_v6_bridge import _proof_module
...
def test_v6_bridge_rejects_hash_drift_before_executing_the_proof_module(tmp_path: Path) -> None:
...
def test_v6_bridge_rejects_an_in_root_symlink_before_executing_the_proof_module(tmp_path: Path) -> None:
    root = tmp_path / "project"
    root.mkdir()
...
def test_v6_bridge_rejects_a_proof_changed_while_its_loader_executes(
    tmp_path: Path,
    ...
    module_name = f"_axon_loop167_v6_{proof_path.stem}_{expected_sha256[:16]}"
    ...
    def spec_from_file_location(module_name: str, path: Path) -> importlib.machinery.ModuleSpec:
        assert module_name.startswith("_axon_loop167_v6_")
    ...
    monkeypatch.setattr(
        loop166_v6_bridge.importlib.util,
