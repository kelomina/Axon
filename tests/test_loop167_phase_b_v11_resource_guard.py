...
import src.loop167_phase_b.resource_guard_v11 as resource_guard_v11
...
from src.loop167_phase_b.windows_job_v11 import WindowsJobAssignmentProbeV11
...
def _ready_probe() -> WindowsJobAssignmentProbeV11:
...
def _resource_fixture(
    ...
    _write_json(
        root,
        SOURCE_CLOSURE_RELATIVE_PATH,
        {"schema": "axon_loop167_phase_b_source_closure_v11", "loop_id": LOOP_ID},
    )
    _write_json(
        root,
        RUNTIME_LOCK_RELATIVE_PATH,
        {"schema": "axon_loop167_phase_b_runtime_lock_v11", "loop_id": LOOP_ID},
    )
    bindings = {
    ...
    monkeypatch.setattr(resource_guard_v11, "verify_execution_contract_v11", verify_contract)
...
def _build_guard(
    ...
    *,
    ...
    return resource_guard_v11.build_resource_guard_payload_v11(
        root,
        ...
        snapshot=resource_guard_v11.SystemResourceSnapshotV11(
...
def _validate_guard(
    ...
    *,
    ...
    resource_guard_v11.validate_resource_guard_payload_v11(
        root,
        payload,
...
def test_v9_resource_guard_accepts_low_available_memory_with_containment_probe(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    payload = _build_guard(root, bindings, available_memory_bytes=1)

    assert payload["snapshot"]["available_memory_bytes"] == 1
...
def test_v9_resource_guard_accepts_fresh_ready_snapshot_without_memory_floor(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    payload = _build_guard(root, bindings, available_memory_bytes=0)
    _write_json(root, RESOURCE_GUARD_RELATIVE_PATH, payload)
    guard_binding = _binding(root, RESOURCE_GUARD_RELATIVE_PATH)

    verified = resource_guard_v11.verify_resource_guard_v11(
        root,
        guard_binding,
        expected_execution_contract_binding=bindings["execution_contract"],
        expected_source_closure_binding=bindings["source_closure"],
...
def test_v9_resource_guard_rejects_a_stale_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    payload = _build_guard(root, bindings, available_memory_bytes=0)
    _write_json(root, RESOURCE_GUARD_RELATIVE_PATH, payload)
    guard_binding = _binding(root, RESOURCE_GUARD_RELATIVE_PATH)
    ...
    with pytest.raises(PhaseBContractError, match="stale"):
        resource_guard_v11.verify_resource_guard_v11(
            root,
            guard_binding,
            expected_execution_contract_binding=bindings["execution_contract"],
            expected_source_closure_binding=bindings["source_closure"],
...
def test_v9_resource_guard_rejects_drifted_static_binding(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    ...
    _write_json(
        ...
        SOURCE_CLOSURE_RELATIVE_PATH,
        {
            "schema": "axon_loop167_phase_b_source_closure_v11",
            "loop_id": LOOP_ID,
            "drift": True,
        },
    )
