...
import src.loop167_phase_b.execution_authorization_v7 as execution_authorization_v7
...
from src.loop167_phase_b.resource_guard_v7 import RESOURCE_GUARD_SCHEMA
...
def _authorization_fixture(
    ...
    _write_json(
        root,
        SOURCE_CLOSURE_RELATIVE_PATH,
        {"schema": "axon_loop167_phase_b_source_closure_v7", "loop_id": LOOP_ID},
    )
    ...
    _write_json(
        ...
        RUNTIME_LOCK_RELATIVE_PATH,
        {
            "schema": "axon_loop167_phase_b_runtime_lock_v7",
            "loop_id": LOOP_ID,
    ...
    monkeypatch.setattr(execution_authorization_v7, "verify_execution_contract_v7", verify_contract)
    monkeypatch.setattr(execution_authorization_v7, "verify_resource_guard_v7", verify_guard)
    authorization_payload = execution_authorization_v7.build_execution_authorization_payload_v7(
        root,
...
def _validate_prelaunch(root: Path, context: dict[str, object]) -> object:
    return execution_authorization_v7.validate_execution_authorization_v7(
        root,
...
def test_v7_authorization_prelaunch_and_attested_child_surfaces(
    ...
    with pytest.raises(PhaseBContractError, match="attested child requires"):
        execution_authorization_v7.validate_execution_authorization_v7(
            root,
            context["authorization_path"],
            now_utc=NOW,
    ...
    monkeypatch.setattr(execution_authorization_v7, "validate_launch_receipt_v7", validate_launch)
    monkeypatch.setattr(execution_authorization_v7, "verify_child_job_attestation_v7", verify_attestation)
    verified = execution_authorization_v7.validate_execution_authorization_v7(
        root,
        context["authorization_path"],
        now_utc=NOW,
        phase="attested_child",
