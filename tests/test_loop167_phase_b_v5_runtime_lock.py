...
from src.loop167_phase_b import runtime_lock_v5
...
def _synthetic_bindings(root: Path) -> tuple[dict[str, str], dict[str, str], Path]:
    ...
    execution_contract = root / EXECUTION_CONTRACT_V5_RELATIVE_PATH
...
def test_runtime_lock_binds_controller_execution_contract_and_both_invocations(
    ...
    monkeypatch.setattr(runtime_lock_v5, "validate_runtime_envelope_v5", lambda root: (executable, root))
    monkeypatch.setattr(runtime_lock_v5, "_require_clean_external_import_state", lambda: None)
    monkeypatch.setattr(runtime_lock_v5, "_python_binding", lambda root: python_binding)
    monkeypatch.setattr(runtime_lock_v5, "_runtime_packages", lambda root: package_bindings)
    for name, value in THREAD_ENVIRONMENT_V5.items():
        monkeypatch.setenv(name, value)

    payload = runtime_lock_v5.build_runtime_lock_payload_v5(
        root,
        controller_binding=controller_binding,
        execution_contract_binding=execution_binding,
    )
    receipt = runtime_lock_v5.validate_runtime_lock_v5(
        root,
        payload,
        controller_binding=controller_binding,
        execution_contract_binding=execution_binding,
    ...
    with pytest.raises(PhaseBContractError):
        runtime_lock_v5.validate_runtime_lock_v5(
            root,
            drifted,
            controller_binding=controller_binding,
            execution_contract_binding=execution_binding,
...
def test_runtime_lock_rejects_an_execution_contract_at_an_unpinned_path(
    ...
    with pytest.raises(PhaseBContractError):
        runtime_lock_v5.build_runtime_lock_payload_v5(
            root,
            controller_binding=controller_binding,
...
def test_lock_builder_pins_threads_before_importing_project_runtime_modules() -> None:
    script_path = Path(__file__).resolve().parents[1] / "scripts" / "build_loop167_phase_b_runtime_lock_v5.py"
