...
import src.loop167_phase_b.preflight_v6 as preflight_v6
...
from src.loop167_phase_b.preflight_v6 import (
    EXPECTED_BLOCKERS,
...
def _preflight_fixture(root: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    _write(root, PARENT_V5_PRELEASE_ATTESTATION_RELATIVE_PATH, canonical_json_bytes({"parent": "synthetic"}))
    ...
    source_payload = {
        ...
        "loop_id": LOOP_ID,
        "scope": V6_SCOPE,
        "parent_v5_prelease_attestation": _binding(root, PARENT_V5_PRELEASE_ATTESTATION_RELATIVE_PATH),
    ...
    monkeypatch.setattr(preflight_v6, "verify_parent_v5_prelease_attestation_v6", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        preflight_v6,
...
def test_v6_prelaunch_attested_and_leased_pre_raw_surfaces_transition_without_raw_access(
    tmp_path: Path,
) -> None:
    root = tmp_path / "project"
    root.mkdir()

    assert_output_catalog_is_fresh_v6(root)
    launch = _output_path(root, "supervisor_launch_receipt")
    ...
    assert_attested_child_prelease_surface_v6(root)
    with pytest.raises(PhaseBContractError, match="Project path is missing"):
        assert_leased_child_pre_raw_surface_v6(root)
...
def test_v6_post_lease_guard_rejects_data_plane_source_drift_before_any_raw_open(
    tmp_path: Path,
    ...
    initial = preflight_v6.validate_static_preflight_v6(
        root,
    ...
    with pytest.raises(PhaseBContractError, match="hash mismatch"):
        preflight_v6.validate_static_preflight_v6(
            root,
            source_closure_binding=bindings["source_closure"],
            controller_binding=bindings["controller"],
            supervisor_binding=bindings["supervisor"],
