...
def _authorization(root: Path, *, digest: str = "a" * 64) -> VerifiedExecutionAuthorizationV6:
...
def _install_verification_mocks(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ValidatedLaunchReceiptV6(
    ...
    monkeypatch.setattr(lease_v6, "validate_execution_authorization_v6", lambda *_args, **_kwargs: authorization)
    real_sha256_file = lease_v6.sha256_file
    monkeypatch.setattr(
        lease_v6,
        "sha256_file",
        lambda path: (
...
def test_v6_lease_is_one_shot_and_binds_the_validated_launch_digest(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    consumed = consume_execution_lease_v6(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)
    verified = verify_consumed_execution_lease_v6(
        root,
        authorization,
        launch_id=LAUNCH_ID,
        now_utc=NOW,
    ...
    with pytest.raises(ExecutionLeaseV6Error, match="already exists"):
        consume_execution_lease_v6(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)


def test_v6_lease_rejects_a_child_attestation_with_a_different_exact_launch_digest(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "project"
    ...
    with pytest.raises(ExecutionLeaseV6Error, match="launch receipt digest drifted"):
        build_execution_lease_payload_v6(authorization, launch_id=LAUNCH_ID, consumed_at_utc=NOW)


def test_v6_lease_revalidates_authorization_after_consumption(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "project"
    ...
    monkeypatch.setattr(lease_v6, "validate_execution_authorization_v6", lambda *_args, **_kwargs: drifted)

    with pytest.raises(ExecutionLeaseV6Error, match="authorization drifted after lease"):
        verify_consumed_execution_lease_v6(root, authorization, launch_id=LAUNCH_ID, now_utc=NOW)


def test_v6_lease_propagates_post_lease_source_or_guard_revalidation_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "project"
...
def test_v6_lease_rejects_a_marker_replaced_after_its_canonical_read(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "project"
    ...
    _install_verification_mocks(monkeypatch, authorization)
    consume_execution_lease_v6(root, authorization.authorization_path, now_utc=NOW, launch_id=LAUNCH_ID)
    original_builder = lease_v6.build_execution_lease_payload_v6
    ...
    monkeypatch.setattr(lease_v6, "build_execution_lease_payload_v6", replace_marker_after_expected_payload)

    with pytest.raises(ExecutionLeaseV6Error, match="marker changed during verification"):
        verify_consumed_execution_lease_v6(root, authorization, launch_id=LAUNCH_ID, now_utc=NOW)
