...
from src.loop167_phase_b.preflight_v2 import validate_static_preflight_v2
...
SOURCE_CLOSURE_PATH = ARTIFACT_ROOT / "phase_b_source_closure_v2.json"
...
def test_v2_static_preflight_opens_no_raw_inputs(monkeypatch: pytest.MonkeyPatch) -> None:
    if not SOURCE_CLOSURE_PATH.is_file():
        pytest.skip("Phase-B source closure v2 is not sealed yet")
    for name, value in REQUIRED_ENVIRONMENT.items():
        monkeypatch.setenv(name, value)

    receipt = validate_static_preflight_v2(
        PROJECT_ROOT,
    ...
    assert len(receipt.source_closure_sha256) == 64
