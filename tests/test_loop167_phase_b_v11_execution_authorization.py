...
import src.loop167_phase_b.execution_authorization_v11 as execution_authorization_v11
...
from src.loop167_phase_b.resource_guard_v11 import RESOURCE_GUARD_SCHEMA
...
def _authorization_fixture(
    ...
    _write_json(
        ...
        RUNTIME_LOCK_RELATIVE_PATH,
        {
            "schema": "axon_loop167_phase_b_runtime_lock_v11",
            "loop_id": LOOP_ID,
    ...
    monkeypatch.setattr(execution_authorization_v11, "verify_execution_contract_v11", verify_contract)
    monkeypatch.setattr(execution_authorization_v11, "verify_resource_guard_v11", verify_guard)
    authorization_payload = execution_authorization_v11.build_execution_authorization_payload_v11(
        root,
...
def _validate_prelaunch(root: Path, context: dict[str, object]) -> object:
    return execution_authorization_v11.validate_execution_authorization_v11(
        root,
...
def test_v7_authorization_prelaunch_and_attested_child_surfaces(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(PhaseBContractError, match="attested child requires"):
        execution_authorization_v11.validate_execution_authorization_v11(
            root,
            context["authorization_path"],
            now_utc=NOW,
    ...
    monkeypatch.setattr(execution_authorization_v11, "validate_launch_receipt_v11", validate_launch)
    monkeypatch.setattr(execution_authorization_v11, "verify_child_job_attestation_v11", verify_attestation)
    verified = execution_authorization_v11.validate_execution_authorization_v11(
        root,
        context["authorization_path"],
        now_utc=NOW,
        phase="attested_child",
...
def test_v7_authorization_rejects_guard_binding_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, context = _authorization_fixture(tmp_path, monkeypatch)
...
def test_v7_authorization_rejects_controller_binding_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, context = _authorization_fixture(tmp_path, monkeypatch)
