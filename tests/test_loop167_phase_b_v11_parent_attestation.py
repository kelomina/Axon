...
import src.loop167_phase_b.execution_contract_v11 as contract_v11
from src.loop167_phase_b.contracts import PhaseBContractError

PROJECT_ROOT = Path(__file__).resolve().parents[1]


def test_v11_parent_attestation_binds_v10_launcher_controller_identity_mismatch() -> None:
    payload = contract_v11.build_parent_v10_prelease_attestation_payload_v11(PROJECT_ROOT)
    ...
    assert payload["parent_v10_execution_contract"]["path"] == contract_v11.PARENT_V10_EXECUTION_CONTRACT_RELATIVE_PATH
    assert payload["v10_child_attestation_source"]["path"] == "src/loop167_phase_b/child_attestation_v10.py"


def test_v11_parent_attestation_rejects_a_parent_contract_with_a_different_output_surface(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "output_catalog": [{"path": path} for path in contract_v11.PARENT_V10_OUTPUT_PATHS[:-1]],
        "lease": dict(contract_v11.PARENT_V10_EXPECTED_LEASE),
        "canonical_controller_execute_argv": [
            contract_v11.VNEV_PYTHON_RELATIVE_PATH,
            "-I",
    ...
    with pytest.raises(PhaseBContractError, match="output catalog drifted"):
        contract_v11._validate_parent_v10_execution_surface(payload)


def test_v11_parent_attestation_rejects_a_required_absence_violation(tmp_path: Path) -> None:
    ...
    with pytest.raises(PhaseBContractError, match="dynamic surface exists"):
        contract_v11._assert_absent(tmp_path, ("forbidden.json",), label="dynamic surface")
