...
import src.loop167_phase_b.execution_contract_v10 as execution_contract_v10
...
def test_v7_control_planes_reject_a_simulated_windows_reparse_point(
    ...
    with pytest.raises(PhaseBContractError, match="symlink or reparse"):
        if surface == "contract":
            execution_contract_v10._require_json_binding(
                root,
                binding,
                label="target",
            ...
            preflight_v10._verify_source_files(root, [binding])
        elif surface == "authorization":
            execution_authorization_v10._binding(
                root,
                binding,
                label="target",
                expected_path=relative_path,
