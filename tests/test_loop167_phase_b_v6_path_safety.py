...
import src.loop167_phase_b.execution_contract_v6 as execution_contract_v6
...
def test_v6_control_planes_reject_a_simulated_windows_reparse_point(
    tmp_path: Path,
    ...
    with pytest.raises(PhaseBContractError, match="symlink or reparse"):
        if surface == "contract":
            execution_contract_v6._require_json_binding(
                root,
                binding,
                label="target",
            ...
            preflight_v6._verify_source_files(root, [binding])
        elif surface == "authorization":
            execution_authorization_v6._binding(
                root,
                binding,
                label="target",
                expected_path=relative_path,
...
def test_v6_execution_contract_rejects_an_in_root_symlink_binding(tmp_path: Path) -> None:
    root = tmp_path / "project"
    root.mkdir()
...
def test_v6_preflight_rejects_an_in_root_symlink_source_file(tmp_path: Path) -> None:
    root = tmp_path / "project"
    root.mkdir()
...
def test_v6_authorization_rejects_an_in_root_symlink_binding(tmp_path: Path) -> None:
    root = tmp_path / "project"
    root.mkdir()
...
def test_v6_resource_guard_rejects_an_in_root_symlink_binding(tmp_path: Path) -> None:
    root = tmp_path / "project"
    root.mkdir()
