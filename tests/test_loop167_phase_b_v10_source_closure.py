...
from seal_loop167_phase_b_source_closure_v10 import V10_SOURCE_PATHS  # noqa: E402

from loop167_phase_b.preflight_v10 import REQUIRED_SOURCE_PATHS  # noqa: E402


def test_v7_source_closure_contains_the_complete_control_and_data_plane() -> None:
    source_paths = set(V10_SOURCE_PATHS)

    assert len(V10_SOURCE_PATHS) == len(source_paths)
...
def test_v7_source_closure_inventory_points_only_to_regular_workspace_files() -> None:
    assert all((PROJECT_ROOT / relative_path).is_file() for relative_path in V10_SOURCE_PATHS)
    assert all(not (PROJECT_ROOT / relative_path).is_symlink() for relative_path in V10_SOURCE_PATHS)
