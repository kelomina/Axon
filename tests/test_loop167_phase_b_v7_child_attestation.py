...
import src.loop167_phase_b.child_attestation_v7 as child_attestation_v7
from src.loop167_phase_b.child_attestation_v7 import (
...
def _validated_launch(root: Path, *, launcher_pid: int) -> ValidatedLaunchReceiptV7:
...
def _install_common_attestation_mocks(
    ...
    *,
    ...
    monkeypatch.setattr(child_attestation_v7, "validate_launch_receipt_v7", lambda *_args, **_kwargs: validated)
    monkeypatch.setattr(
        child_attestation_v7,
    ...
    monkeypatch.setattr(child_attestation_v7, "_verify_live_process_identity", verify_supervisor)
    monkeypatch.setattr(child_attestation_v7, "validate_spawn_lineage", lambda *_args, **_kwargs: lineage)
    monkeypatch.setenv("AXON_LOOP167_V7_SUPERVISOR_PID", str(SUPERVISOR_PID))
    return observed
...
def test_child_attestation_binds_the_validator_exact_bytes_digest_and_direct_lineage(
    ...
    payload = build_child_job_attestation_payload_v7(
        tmp_path,
...
def _install_identity_kernel(monkeypatch: pytest.MonkeyPatch, *, wait_result: int, creation: int) -> None:
    ...
    monkeypatch.setattr(child_attestation_v7.os, "name", "nt")
