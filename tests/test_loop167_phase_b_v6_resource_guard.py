...
import src.loop167_phase_b.resource_guard_v6 as resource_guard_v6
...
from src.loop167_phase_b.windows_job_v6 import WindowsJobAssignmentProbeV6
...
def _ready_probe() -> WindowsJobAssignmentProbeV6:
...
def _resource_fixture(
    ...
    _write_json(
        root,
        RUNTIME_LOCK_RELATIVE_PATH,
        {"schema": "axon_loop167_phase_b_runtime_lock_v6", "loop_id": LOOP_ID},
    )
    bindings = {
    ...
    monkeypatch.setattr(resource_guard_v6, "verify_execution_contract_v6", verify_contract)
...
def _build_guard(
    ...
    *,
    ...
    return resource_guard_v6.build_resource_guard_payload_v6(
        root,
        ...
        snapshot=resource_guard_v6.SystemResourceSnapshotV6(
...
def _validate_guard(
    ...
    *,
    ...
    resource_guard_v6.validate_resource_guard_payload_v6(
        root,
        payload,
...
def test_v6_resource_guard_fails_closed_below_twelve_gib(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_v6_resource_guard_accepts_fresh_ready_twelve_gib_snapshot(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    ...
    verified = resource_guard_v6.verify_resource_guard_v6(
        root,
        guard_binding,
        expected_execution_contract_binding=bindings["execution_contract"],
        expected_source_closure_binding=bindings["source_closure"],
...
def test_v6_resource_guard_rejects_a_stale_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    payload = _build_guard(root, bindings, available_memory_bytes=12 * GIBIBYTE)
    _write_json(root, RESOURCE_GUARD_RELATIVE_PATH, payload)
    guard_binding = _binding(root, RESOURCE_GUARD_RELATIVE_PATH)
    ...
    with pytest.raises(PhaseBContractError, match="stale"):
        resource_guard_v6.verify_resource_guard_v6(
            root,
            guard_binding,
            expected_execution_contract_binding=bindings["execution_contract"],
            expected_source_closure_binding=bindings["source_closure"],
...
def test_v6_resource_guard_rejects_drifted_static_binding(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root, bindings = _resource_fixture(tmp_path, monkeypatch)
    ...
    _write_json(
        ...
        SOURCE_CLOSURE_RELATIVE_PATH,
        {
            "schema": "axon_loop167_phase_b_source_closure_v6",
            "loop_id": LOOP_ID,
            "drift": True,
        },
    )
