...
SUPERVISOR_ENTRY_PATH = PROJECT_ROOT / "scripts" / "run_loop167_phase_b_supervisor_v8.py"


def _load_supervisor_entry():
    specification = importlib.util.spec_from_file_location(
        "loop167_phase_b_supervisor_v8_entry_test",
...
def test_v8_supervisor_entry_passes_sealed_relative_controller_argv(monkeypatch) -> None:
    ...
    monkeypatch.setattr(supervisor_entry, "create_fresh_launch_authorization_v8", lambda *_args, **_kwargs: fresh_launch)
    monkeypatch.setattr(
        supervisor_entry,
