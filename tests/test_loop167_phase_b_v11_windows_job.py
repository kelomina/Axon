...
from src.loop167_phase_b import windows_job_v11
...
class FakeApi:
    ...
    def assign_process(self, job_handle, process_handle):
        ...
        if self.assignment_error is not None:
            raise windows_job_v11.WindowsJobV11Error(
...
def test_controller_job_uses_memory_and_kill_on_close_flags():
    expected_flags = (
        windows_job_v11.JOB_OBJECT_LIMIT_PROCESS_MEMORY
        | windows_job_v11.JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
    )
    api = FakeApi(limit_flags=expected_flags)

    job = windows_job_v11.WindowsJobV11.create(
...
def test_nonkill_probe_performs_real_assignment_and_records_error_code():
    expected_flags = windows_job_v11.JOB_OBJECT_LIMIT_PROCESS_MEMORY
    api = FakeApi(limit_flags=expected_flags, assignment_error=6)

    result = windows_job_v11.probe_windows_job_assignment_v11(
        memory_limit_bytes=4096,
        job_factory=lambda **kwargs: windows_job_v11.WindowsJobV11.create(api=api, **kwargs),
    )
...
def test_create_rejects_flag_drift_and_closes_handle():
    api = FakeApi(limit_flags=windows_job_v11.JOB_OBJECT_LIMIT_PROCESS_MEMORY)

    with pytest.raises(windows_job_v11.WindowsJobV11Error, match="unexpected limit flags"):
        windows_job_v11.WindowsJobV11.create(
            memory_limit_bytes=4096,
            kill_on_close=True,
            api=api,
        )
...
def test_failed_child_membership_terminates_without_resume(tmp_path):
    ...
    with pytest.raises(windows_job_v11.WindowsJobV11Error, match="returned false"):
...
def test_kernel32_api_declares_handle_safe_signatures(monkeypatch):
    ...
    monkeypatch.setattr(windows_job_v11.os, "name", "nt")
    monkeypatch.setattr(windows_job_v11.ctypes, "WinDLL", fake_windll, raising=False)
    api = windows_job_v11._Kernel32JobApiV11()

    assert observed == {"name": "kernel32", "use_last_error": True}
    assert api.kernel32.GetCurrentProcess.argtypes == []
    assert api.kernel32.GetCurrentProcess.restype == windows_job_v11.wintypes.HANDLE
    assert api.kernel32.AssignProcessToJobObject.argtypes == [
        windows_job_v11.wintypes.HANDLE,
        windows_job_v11.wintypes.HANDLE,
    ]
    assert api.kernel32.AssignProcessToJobObject.restype == windows_job_v11.wintypes.BOOL
...
def test_current_process_accepts_the_valid_minus_one_pseudohandle(monkeypatch):
    ...
    api = object.__new__(windows_job_v11._Kernel32JobApiV11)
