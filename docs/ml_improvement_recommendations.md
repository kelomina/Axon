...
| --- | --- |
| 无扩展名白样本 | `<none>` 扩展错误 `1261`，其中 FP `1234` |
...
1. **补无扩展名良性文件的身份特征。** 当前大量白样本文件名是 SHA 或无扩展名，模型容易把它们判黑。建议显式加入 filename/extension bucket、PE 是否可执行入口、签名/证书、导入表可信度、overlay/packer 风险分层等特征，并单独建立 high-value benign holdout。
...
当前科学判断：`99.9%` 不是短期阈值或小清洗能达到的指标。若不引入更强特征、更严格噪声治理和多模型 OOF stacking，合理目标应先降为 `98.5%-99.0%` full-test F1，并把 FP/FN 分业务成本分别设门槛。继续挑战 `99.9%` 可以保留为长期目标，但需要把“数据标签可信度”和“PE/DLL/sys/无扩展名覆盖”作为前置工程。
