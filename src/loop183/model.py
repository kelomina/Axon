"""Independent Multi-Scale HGConv-Region model for Loop183.

架构与 Loop182 一致：
- byte_embedding → patchify (Conv1d) → MultiScaleHGConvBlocks → attention pooling
- → region_transformer → fusion_head
Loop183 唯一改动：默认 dropout 从 0.1 提升到 0.2（与 contracts.DROPOUT 对齐）。
"""
...
class HGConvRegionConfig:
    """Loop183 模型配置（与 Loop182 一致 + dropout 0.1 → 0.2）。"""
    ...
    dropout: float = 0.2  # Loop183: 0.1 → 0.2
...
class HGConvRegionNet(nn.Module):
    """Loop183 Multi-Scale HGConv-Region 模型（与 Loop182 架构一致）。"""
