"""PyTorch Multi-Scale HGConv core for the Loop183 lineage.

架构与 Loop182 完全一致：多尺度并行 HGConvBlock（filter_lengths=[8, 16, 32, 64]）。
Loop183 改进聚焦于训练时的正则化和数据增强，不在架构层面改动。
"""
...
class HGConvConfig:
    """单尺度 HGConv 配置（保留用于回退对比，Loop183 不使用）。"""
...
class HGConvBlock(nn.Module):
    """单尺度 HGConvBlock（保留用于回退对比，Loop183 不使用）。"""
...
class MultiScaleHGConvBlock(nn.Module):
    """多尺度并行 HGConvBlock（与 Loop182 架构一致）。
