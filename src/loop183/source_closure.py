"""Loop183 源码闭包验证器（与 Loop182 一致，仅修改扫描路径）。"""
...
_LOOP183_DIR: Final[Path] = _PROJECT_ROOT / "src" / "loop183"
...
class ClosureReport:
    ...
    def assert_passed(self) -> None:
        if not self.passed:
            lines = ["Loop183 source closure failed:"]
...
def scan_source_closure(
    *,
    ...
    for path in _LOOP183_DIR.rglob("*.py"):
