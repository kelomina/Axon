"""Independent Multi-Scale HGConv-Region model for Loop184.

Loop184 架构改进（vs Loop183）：
- byte_embedding → patchify (Conv1d) → 2 × MultiScaleHGConvBlocks → attention pooling
- → 4 层 region_transformer（vs Loop183 的 2 层）→ fusion_head

参数量估算：1.62M → ~2.17M（+33%）
- 增加 1 个 MultiScaleHGConvBlock: +98,692
- 增加 2 层 TransformerEncoderLayer: +889,728

实际增加：~988,420，总计约 2,610,573
"""
...
class HGConvRegionNet(nn.Module):
    """Loop184 Multi-Scale HGConv-Region 模型（架构扩展版）。"""
    ...
    def __init__(self, config: HGConvRegionConfig | None = None) -> None:
        ...
        # Loop184 改进：2 个 MultiScaleHGConvBlock
        ...
        # Loop184 改进：4 层 TransformerEncoder
