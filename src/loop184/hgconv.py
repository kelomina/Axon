"""PyTorch Multi-Scale HGConv core for the Loop184 lineage.

架构与 Loop183 一致：多尺度并行 HGConvBlock（filter_lengths=[8, 16, 32, 64]）。
Loop184 改进聚焦于训练时 SAM + SWA + 架构扩展（2 个 hgconv blocks, 4 层 transformer），
不在 HGConv block 层面改动。
"""
...
class HGConvConfig:
    """单尺度 HGConv 配置（保留用于回退对比，Loop184 不使用）。"""
...
class HGConvBlock(nn.Module):
    """单尺度 HGConvBlock（保留用于回退对比，Loop184 不使用）。"""
...
class MultiScaleHGConvBlock(nn.Module):
    """多尺度并行 HGConvBlock（与 Loop183 架构一致）。
