"""Loop184 数据适配器（与 Loop183 一致，仅修改 schema 名和 import 路径）。
...
class PhaseADataLoader:
    """Phase A 数据加载器（与 Loop183 一致，复用 Loop175 region cache）。"""
    ...
    def load_real_data(
        ...
        *,
        ...
        return {
            "schema": "axon_loop184_phase_a_data_receipt_v1",
    ...
    def materialize_batch(self, row_indices: np.ndarray) -> dict[str, object]:
        ...
        return {
            ...
            "file_sizes": torch.from_numpy(file_sizes[row_indices].astype(np.int64, copy=False)),
        }
