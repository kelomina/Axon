"""Loop184 源码闭包验证器（与 Loop183 一致，仅修改扫描路径）。"""
...
_LOOP184_DIR: Final[Path] = _PROJECT_ROOT / "src" / "loop184"
...
class ClosureReport:
    ...
    def assert_passed(self) -> None:
        if not self.passed:
            lines = ["Loop184 source closure failed:"]
...
def scan_source_closure(
    *,
    ...
    for path in _LOOP184_DIR.rglob("*.py"):
