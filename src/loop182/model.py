"""Independent Multi-Scale HGConv-Region model shell for Loop182.

Loop182 = Loop179 架构 + MultiScaleHGConvBlock 替换 HGConvBlock。
其余结构（patchify、attention pooling、region transformer、fusion head）与 Loop179 一致。
"""
...
class HGConvRegionConfig:
    """Loop182 模型配置（与 Loop179 一致 + 多尺度 filter_lengths）。"""
    ...
    patch_size: int = 16
    hgconv_blocks: int = 1
    # Loop182 核心改进：多尺度 filter_lengths（覆盖单尺度 filter_length 的语义）
...
class HGConvRegionNet(nn.Module):
    """Loop182 Multi-Scale HGConv-Region 模型。"""
    ...
    def __init__(self, config: HGConvRegionConfig | None = None) -> None:
        ...
        # Loop182 核心改进：使用 MultiScaleHGConvBlock
