"""Independent HGConv-Region model shell for Loop179."""
...
from .hgconv import HGConvBlock, HGConvConfig
...
class HGConvRegionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.expected_region_bytes // self.patch_size < self.hgconv_filter_length:
            raise ValueError("patch sequence must not be shorter than the HGConv filter")
...
class HGConvRegionNet(nn.Module):
    def __init__(self, config: HGConvRegionConfig | None = None) -> None:
        ...
        block_config = HGConvConfig(
            model_dim=cfg.model_dim,
            filter_length=cfg.hgconv_filter_length,
            dropout=cfg.dropout,
        )
        self.blocks = nn.ModuleList(HGConvBlock(block_config) for _ in range(cfg.hgconv_blocks))
