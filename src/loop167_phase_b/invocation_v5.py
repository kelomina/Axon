"""Canonical isolated-Windows invocation contract for Loop167 Phase-B v5."""
...
CONTROLLER_V5_RELATIVE_PATH = "scripts/run_loop167_phase_b_controller_v5.py"
EXECUTION_CONTRACT_V5_RELATIVE_PATH = (
    "manifests/roadmap_9997/loop167_ember_v3_novel_delta/"
    "phase_b_v5_mappingproxy_remediation/phase_b_execution_contract_v5.json"
)
...
class VerifiedRuntimeInvocationV5:
    """The isolated controller process that is permitted to enter the v5 chain."""
...
def canonical_argv_v5(mode: str) -> tuple[str, ...]:
    """Return the only permitted supervisor argv for a v5 controller mode."""
...
def canonical_controller_argv_v5(mode: str) -> tuple[str, ...]:
    """Return the exact ``sys.argv`` expected after the canonical Python launch."""

    normalized_mode = _require_mode(mode)
    return (CONTROLLER_V5_RELATIVE_PATH, MODE_FLAGS[normalized_mode])


def canonical_argv_hashes_v5() -> dict[str, str]:
    return {mode: canonical_argv_sha256(canonical_argv_v5(mode)) for mode in MODE_FLAGS}
...
def bootstrap_thread_environment_v5(
    *,
    ...
    target_environment.update(THREAD_ENVIRONMENT_V5)
    return dict(THREAD_ENVIRONMENT_V5)


def validate_thread_environment_v5(environment: Mapping[str, str] | None = None) -> None:
    observed_environment = os.environ if environment is None else environment
    if any(observed_environment.get(name) != value for name, value in THREAD_ENVIRONMENT_V5.items()):
        raise PhaseBContractError("Runtime thread environment is not pinned before package import")


def validate_launch_argv_v5(argv: Sequence[str], *, mode: str) -> None:
    if isinstance(argv, (str, bytes)) or tuple(argv) != canonical_argv_v5(mode):
        raise PhaseBContractError("Runtime launch argv does not match the canonical v5 command")
...
def validate_runtime_envelope_v5(
    root: Path | str,
    *,
    ...
    if observed_os_name != "nt":
        raise PhaseBContractError("Loop167 Phase-B v5 may run only under Windows")
    ...
    if observed_relative != VNEV_PYTHON_RELATIVE_PATH or not _same_path(
        ...
        raise PhaseBContractError("Loop167 Phase-B v5 must use the project vnev Python executable")
    observed_cwd = Path.cwd() if cwd is None else Path(cwd)
    if not observed_cwd.is_absolute():
        raise PhaseBContractError("Loop167 Phase-B v5 cwd must be absolute")
    observed_cwd = safe_project_root(observed_cwd)
    if not _same_path(observed_cwd, root_path):
        raise PhaseBContractError("Loop167 Phase-B v5 cwd must be the project root")
    observed_isolated = bool(sys.flags.isolated) if isolated is None else isolated
    if observed_isolated is not True:
        raise PhaseBContractError("Loop167 Phase-B v5 requires Python -I isolated mode")
    validate_thread_environment_v5(environment)
    return expected_executable, root_path


def validate_current_runtime_invocation_v5(
    root: Path | str,
    *,
    mode: str,
    executable: Path | str | None = None,
    ...
    environment: Mapping[str, str] | None = None,
) -> VerifiedRuntimeInvocationV5:
    """Validate the active controller invocation after stdlib-only bootstrap."""

    normalized_mode = _require_mode(mode)
    executable_path, root_path = validate_runtime_envelope_v5(
        root,
    ...
    expected_controller_argv = canonical_controller_argv_v5(normalized_mode)
    if observed_argv != expected_controller_argv:
        raise PhaseBContractError("Controller sys.argv does not match the canonical v5 invocation")
    canonical_argv = canonical_argv_v5(normalized_mode)
    return VerifiedRuntimeInvocationV5(
