"""Raw-free static validation for the Loop167 Phase-B v11 control plane."""
...
SOURCE_CLOSURE_SCHEMA = "axon_loop167_phase_b_source_closure_v11"
V11_SCOPE = "v8_in_process_fresh_authorization_remediation_single_authorized_train_only_execution_no_heldout_access"
...
REQUIRED_SOURCE_PATHS = frozenset(
    {
        ...
        "src/loop167_phase_b/execution_contract_v11.py",
        "src/loop167_phase_b/windows_job_v11.py",
        "src/loop167_phase_b/supervisor_v11.py",
        "src/loop167_phase_b/invocation_v11.py",
        "src/loop167_phase_b/runtime_lock_v11.py",
        "src/loop167_phase_b/preflight_v11.py",
        "src/loop167_phase_b/resource_guard_v11.py",
        "src/loop167_phase_b/execution_authorization_v11.py",
        "src/loop167_phase_b/launch_authorization_v11.py",
        "src/loop167_phase_b/lease_v11.py",
        "src/loop167_phase_b/child_attestation_v11.py",
        "src/loop167_phase_b/loop166_v8_bridge.py",
        "src/loop166/windows_job.py",
        "src/loop166/windows_process_lineage.py",
        "scripts/run_loop167_phase_b_supervisor_v11.py",
        "scripts/run_loop167_phase_b_controller_v11.py",
    }
)
...
def _verify_source_files(root: Path, source_files: object) -> None:
    if not isinstance(source_files, list) or not source_files:
        raise PhaseBContractError("v11 source closure has no source-file bindings")
    ...
    for binding in source_files:
        ...
        if relative in observed:
            raise PhaseBContractError("v11 source closure repeats a source file")
        observed.add(relative)
    if not REQUIRED_SOURCE_PATHS.issubset(observed):
        raise PhaseBContractError("v11 source closure omits required control-plane sources")
...
def _validate_source_closure(
    ...
    *,
    ...
    if set(payload) != expected_keys or payload.get("schema") != SOURCE_CLOSURE_SCHEMA:
        raise PhaseBContractError("v11 source closure schema drifted")
    if payload.get("loop_id") != LOOP_ID or payload.get("scope") != V11_SCOPE:
        raise PhaseBContractError("v11 source closure identity drifted")
    if payload.get("static_preflight_ready") is not True or payload.get("phase_b_raw_execution_ready") is not False:
        raise PhaseBContractError("v11 source closure readiness drifted")
    if payload.get("dynamic_execution_gates") != EXPECTED_DYNAMIC_GATES:
        raise PhaseBContractError("v11 source closure dynamic gates drifted")
    if payload.get("remaining_execution_blockers") != EXPECTED_BLOCKERS:
        raise PhaseBContractError("v11 source closure blockers drifted")
    ...
    if _binding(payload.get("controller"), label="controller", expected_path=CONTROLLER_RELATIVE_PATH) != dict(controller_binding):
        raise PhaseBContractError("v11 source closure controller binding drifted")
    if _binding(payload.get("supervisor"), label="supervisor", expected_path=SUPERVISOR_RELATIVE_PATH) != dict(supervisor_binding):
        raise PhaseBContractError("v11 source closure supervisor binding drifted")
...
def _source_file_binding(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise PhaseBContractError(f"v11 source closure lacks a unique {label} binding")
    _path, digest = verify_safe_file_binding(root, candidates[0], label=label)
    return {"path": expected_path, "sha256": digest}


def validate_static_preflight_v11(
    root: Path | str,
    *,
    source_closure_binding: Mapping[str, str],
    controller_binding: Mapping[str, str],
    ...
    if phase not in {"prelaunch", "contained_child", "attested_child", "leased_child_pre_raw"}:
        raise PhaseBContractError("v11 static preflight phase is invalid")
    ...
    if sha256_file(_controller_path) != normalized_controller["sha256"]:
        raise PhaseBContractError("v11 controller changed during static preflight")
    if sha256_file(_supervisor_path) != normalized_supervisor["sha256"]:
        raise PhaseBContractError("v11 supervisor changed during static preflight")
    ...
    verify_parent_v10_prelease_attestation_v11(root_path, closure["parent_v10_prelease_attestation"])
    contract = verify_execution_contract_v11(root_path, closure["phase_b_execution_contract"])
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v11" or runtime_lock.get("loop_id") != LOOP_ID:
        raise PhaseBContractError("v11 runtime lock identity drifted")
    if runtime_lock.get("controller") != normalized_controller or runtime_lock.get("supervisor") != normalized_supervisor:
        raise PhaseBContractError("v11 runtime lock controller bindings drifted")
    if runtime_lock.get("execution_contract") != dict(closure["phase_b_execution_contract"]):
        raise PhaseBContractError("v11 runtime lock execution contract binding drifted")
    if phase == "prelaunch":
        assert_output_catalog_is_fresh_v11(root_path, contract.output_catalog)
    elif phase == "contained_child":
        assert_contained_child_prelease_surface_v11(root_path, contract.output_catalog)
    elif phase == "attested_child":
        assert_attested_child_prelease_surface_v11(root_path, contract.output_catalog)
    else:
        assert_leased_child_pre_raw_surface_v11(root_path, contract.output_catalog)
