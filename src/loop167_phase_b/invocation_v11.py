"""Canonical isolated-Python invocations for the Loop167 Phase-B v11 route."""
...
class VerifiedRuntimeInvocationV11:
    role: str
    mode: str
...
def _mode(mode: str) -> str:
    if mode not in MODE_FLAGS:
        raise PhaseBContractError("v11 mode must be preflight or execute")
    return mode


def _role(role: str) -> str:
    if role not in {"supervisor", "controller"}:
        raise PhaseBContractError("v11 role must be supervisor or controller")
    return role


def canonical_argv_v11(role: str, mode: str) -> tuple[str, ...]:
...
def canonical_process_argv_v11(role: str, mode: str) -> tuple[str, ...]:
    normalized_role = _role(role)
    normalized_mode = _mode(mode)
    script = SUPERVISOR_RELATIVE_PATH if normalized_role == "supervisor" else CONTROLLER_RELATIVE_PATH
...
def canonical_argv_hashes_v11() -> dict[str, str]:
    return {
        f"{role}_{mode}": canonical_argv_sha256(canonical_argv_v11(role, mode))
...
def bootstrap_thread_environment_v11(
    *,
    ...
    if loaded:
        raise PhaseBContractError("v11 thread bootstrap ran after external imports: " + ", ".join(loaded))
    target = os.environ if environment is None else environment
    target.update(THREAD_ENVIRONMENT_V11)
    return dict(THREAD_ENVIRONMENT_V11)


def validate_thread_environment_v11(environment: Mapping[str, str] | None = None) -> None:
    observed = os.environ if environment is None else environment
    if any(observed.get(name) != value for name, value in THREAD_ENVIRONMENT_V11.items()):
        raise PhaseBContractError("v11 thread environment is not pinned")


def validate_runtime_envelope_v11(
    root: Path | str,
    *,
    ...
    if (os.name if os_name is None else os_name) != "nt":
        raise PhaseBContractError("v11 runs only under Windows")
    ...
    if observed_relative != VNEV_PYTHON_RELATIVE_PATH:
        raise PhaseBContractError("v11 requires the project vnev Python executable")
    ...
    if not observed_cwd.is_absolute() or safe_project_root(observed_cwd) != root_path:
        raise PhaseBContractError("v11 requires the canonical project root cwd")
    if (bool(sys.flags.isolated) if isolated is None else isolated) is not True:
        raise PhaseBContractError("v11 requires Python isolated mode")
    validate_thread_environment_v11(environment)
    return expected_executable, root_path


def validate_current_runtime_invocation_v11(
    root: Path | str,
    *,
    role: str,
    mode: str,
    ...
    environment: Mapping[str, str] | None = None,
) -> VerifiedRuntimeInvocationV11:
    normalized_role = _role(role)
    normalized_mode = _mode(mode)
    executable_path, root_path = validate_runtime_envelope_v11(
        root,
    ...
    if observed_argv != canonical_process_argv_v11(normalized_role, normalized_mode):
        raise PhaseBContractError("v11 process argv differs from the sealed invocation")
    argv = canonical_argv_v11(normalized_role, normalized_mode)
    return VerifiedRuntimeInvocationV11(
