"""Raw-free static validation for the Loop167 Phase-B v4 controller.
...
SOURCE_CLOSURE_V3_RELATIVE_PATH = (
    "manifests/roadmap_9997/loop167_ember_v3_novel_delta/phase_b_source_closure_v3.json"
)
PROTOCOL_ADDITION_RELATIVE_PATH = (
    "manifests/roadmap_9997/loop167_ember_v3_novel_delta/phase_b_protocol_addendum.json"
)
V4_SCOPE = "v4_static_preflight_and_single_authorized_train_only_execution_no_heldout_access"
...
EXPECTED_BLOCKERS = [
    "fresh_resource_guard_v4_not_sealed",
    "run_authorization_v4_not_sealed",
    "one_shot_lease_v4_not_consumed",
]
FORBIDDEN_CLOSURE_ARTIFACT_PATHS = frozenset(
    {
        "manifests/roadmap_9997/loop167_ember_v3_novel_delta/phase_b_resource_guard_v4.json",
        "manifests/roadmap_9997/loop167_ember_v3_novel_delta/phase_b_run_authorization.json",
...
class StaticPreflightV4Receipt:
...
def _require_binding_path(binding: object, *, label: str, expected_path: str) -> Mapping[str, str]:
    ...
    if binding.get("path") != expected_path:
        raise PhaseBContractError(f"{label} path drifted from the fixed v4 route")
...
def _validate_v3_provenance(root: Path, binding: object) -> None:
    _require_binding_path(
        binding,
        label="supersedes_source_closure_v3",
        expected_path=SOURCE_CLOSURE_V3_RELATIVE_PATH,
    )
    path, _ = verify_file_binding(root, binding, label="source_closure_v3")
    payload = require_canonical_json(path)
    if payload.get("schema") != "axon_loop167_phase_b_source_closure_v3":
        raise PhaseBContractError("Superseded source closure is not the sealed v3 provenance")
...
def _validate_protocol_and_addendum(
    ...
    *,
    ...
    if protocol.get("phase_a_bindings") != phase_a_bindings:
        raise PhaseBContractError("Phase-A bindings differ between protocol and v4 closure")
...
def _validate_runtime_lock_static_v4(
    root: Path,
    *,
    runtime_lock_binding: Mapping[str, str],
    controller_binding: Mapping[str, str],
    ...
    lock_path, lock_sha256 = verify_file_binding(root, runtime_lock_binding, label="runtime_lock_v4")
    ...
    if lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v4" or lock.get("loop_id") != LOOP_ID:
        raise PhaseBContractError("Runtime lock v4 schema or loop id drifted")
    if lock.get("runtime_platform") != "windows":
        raise PhaseBContractError("Runtime lock v4 platform drifted")
    ...
    expected_argv = {mode: list(canonical_argv_v4(mode)) for mode in ("preflight", "execute")}
    if lock.get("canonical_argv") != expected_argv:
        raise PhaseBContractError("Runtime lock v4 canonical argv drifted")
    if lock.get("canonical_argv_sha256") != canonical_argv_hashes_v4():
        raise PhaseBContractError("Runtime lock v4 canonical argv hashes drifted")
    if tuple(canonical_preflight_argv) != canonical_argv_v4("preflight"):
        raise PhaseBContractError("Static preflight argv is not the fixed v4 invocation")
    if lock.get("thread_environment") != THREAD_ENVIRONMENT_V4:
    ...
    if (
        ...
        raise PhaseBContractError("Runtime lock v4 execution policy drifted")
    ...
    if set(python_binding) != {"relative_path", "sha256", "implementation", "version"}:
        raise PhaseBContractError("Runtime lock v4 Python binding fields drifted")
    if python_binding.get("relative_path") != VNEV_PYTHON_RELATIVE_PATH:
        raise PhaseBContractError("Runtime lock v4 Python path drifted")
    ...
    if not isinstance(packages, list) or len(packages) != 5:
        raise PhaseBContractError("Runtime lock v4 package inventory drifted")
    expected_packages = {
        ("numpy", "numpy"),
        ("scipy", "scipy"),
    ...
    for package in packages:
        ...
        if set(package_binding) != {"distribution", "module", "relative_path", "sha256", "version"}:
            raise PhaseBContractError("Runtime lock v4 package binding fields drifted")
        ...
        if not isinstance(distribution, str) or not isinstance(module, str):
            raise PhaseBContractError("Runtime lock v4 package identity is invalid")
        ...
        if not isinstance(relative_path, str) or not relative_path.startswith("vnev/"):
            raise PhaseBContractError("Runtime lock v4 package escapes the vnev")
        ...
        if not isinstance(package_binding.get("version"), str) or not package_binding["version"]:
            raise PhaseBContractError("Runtime lock v4 package version is invalid")
...
def _validate_source_files(
    ...
    *,
    ...
    if not isinstance(source_files, list) or not source_files:
        raise PhaseBContractError("Phase-B source closure v4 has no source files")
    observed_paths: set[str] = set()
    for source_binding in source_files:
        path, _ = verify_file_binding(root, source_binding, label="source_file_v4")
        relative_path = path.relative_to(root.resolve(strict=True)).as_posix()
        if relative_path in observed_paths:
            raise PhaseBContractError("Phase-B source closure v4 repeats a source file")
        if relative_path in FORBIDDEN_CLOSURE_ARTIFACT_PATHS:
            raise PhaseBContractError("Source closure v4 must not bind dynamic guard, authorization, lease, or outputs")
        observed_paths.add(relative_path)
    if dict(controller_binding) not in source_files:
        raise PhaseBContractError("Controller v4 is not bound by the source closure")
    if "src/loop167_phase_b/preflight_v4.py" not in observed_paths:
        raise PhaseBContractError("Source closure v4 omits the static preflight module")
    if "src/loop167_phase_b/__init__.py" not in observed_paths:
        raise PhaseBContractError("Source closure v4 omits the lazy package boundary")


def validate_static_preflight_v4(
    root: Path,
    *,
    source_closure_binding: Mapping[str, str],
    controller_binding: Mapping[str, str],
    canonical_preflight_argv: Sequence[str],
) -> StaticPreflightV4Receipt:
    """Validate static provenance without touching raw inputs or outputs."""

    _require_binding_path(
        source_closure_binding,
        label="source_closure_v4",
        expected_path=SOURCE_CLOSURE_RELATIVE_PATH,
    )
    _require_binding_path(
        controller_binding,
        label="controller_v4",
        expected_path=CONTROLLER_V4_RELATIVE_PATH,
    )
    closure_path, closure_sha256 = verify_file_binding(root, source_closure_binding, label="source_closure_v4")
    ...
    if set(closure) != expected_keys:
        raise PhaseBContractError("Phase-B source closure v4 fields drifted")
    if closure.get("schema") != "axon_loop167_phase_b_source_closure_v4" or closure.get("loop_id") != LOOP_ID:
        raise PhaseBContractError("Phase-B source closure v4 schema or loop id drifted")
    if closure.get("scope") != V4_SCOPE:
        raise PhaseBContractError("Phase-B source closure v4 scope drifted")
    if closure.get("static_preflight_ready") is not True or closure.get("phase_b_raw_execution_ready") is not False:
        raise PhaseBContractError("Phase-B source closure v4 execution state drifted")
    if closure.get("dynamic_execution_gates") != EXPECTED_DYNAMIC_GATES:
        raise PhaseBContractError("Phase-B source closure v4 dynamic-gate contract drifted")
    if closure.get("remaining_execution_blockers") != EXPECTED_BLOCKERS:
        raise PhaseBContractError("Phase-B source closure v4 blocker contract drifted")

    _validate_v3_provenance(root, closure["supersedes_source_closure_v3"])
    ...
    verified_contract: VerifiedExecutionContractV4 = verify_execution_contract_v4(
        root,
        execution_contract_binding,
        expected_protocol_binding=closure["phase_b_protocol"],
    )
    runtime_lock_binding = _require_binding_path(
        closure["runtime_lock_v4"],
        label="runtime_lock_v4",
        expected_path=RUNTIME_LOCK_RELATIVE_PATH,
    )
    runtime_lock_sha256 = _validate_runtime_lock_static_v4(
        root,
    ...
    _, controller_sha256 = verify_file_binding(root, controller_binding, label="controller_v4")
    return StaticPreflightV4Receipt(
        ...
        controller_binding={"path": CONTROLLER_V4_RELATIVE_PATH, "sha256": controller_sha256},
        raw_open_attempts=0,
    )
