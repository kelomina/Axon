"""Create the v11 dynamic launch artifacts in the supervisor invocation only."""
...
from .resource_guard_v11 import build_resource_guard_payload_v11, current_system_snapshot_v11

MAXIMUM_FRESH_AUTHORIZATION_CHAIN_SECONDS = 30


class FreshAuthorizationV9Error(PhaseBContractError):
    """The v11 dynamic authorization chain cannot prove a fresh launch state."""


@dataclass(frozen=True)
class FreshLaunchAuthorizationV11:
...
def _fsync_parent_directory(parent: Path) -> None:
    if os.name == "nt":
        ...
        if handle in (None, 0, invalid):
            raise FreshAuthorizationV9Error("v11 dynamic artifact parent is unavailable")
        try:
            if not kernel32.FlushFileBuffers(handle):
                raise FreshAuthorizationV9Error("v11 dynamic artifact parent durable sync failed")
        finally:
...
def _write_new(root: Path, relative_path: str, payload: Mapping[str, object]) -> str:
    output_path = ensure_v11_static_artifact_parent(root, relative_path)
    try:
        stat_result = output_path.lstat()
    except FileNotFoundError:
        stat_result = None
    if stat_result is not None or output_path.is_symlink():
        raise FreshAuthorizationV9Error("v11 dynamic artifact already exists or is unsafe")
    ...
    try:
        ...
        raise FreshAuthorizationV9Error("v11 dynamic artifact already exists") from error
    except OSError as error:
        raise FreshAuthorizationV9Error("v11 dynamic artifact cannot be created") from error
...
def create_fresh_launch_authorization_v11(
    root: Path | str,
    *,
    now_utc: datetime,
) -> FreshLaunchAuthorizationV11:
    ...
    if not isinstance(now_utc, datetime) or now_utc.tzinfo is None:
        raise FreshAuthorizationV9Error("v11 fresh authorization requires a UTC timestamp")
    ...
    for relative_path in (RESOURCE_GUARD_RELATIVE_PATH, RUN_AUTHORIZATION_RELATIVE_PATH):
        ...
        if path.exists() or path.is_symlink():
            raise FreshAuthorizationV9Error("v11 execute refuses pre-created dynamic artifacts")
    created_at = now_utc.astimezone(UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    guard_payload = build_resource_guard_payload_v11(
        root_path,
        ...
        snapshot=current_system_snapshot_v11(),
        created_at_utc=created_at,
    )
    if guard_payload.get("guard_ready") is not True:
        raise FreshAuthorizationV9Error("v11 fresh resource guard failed closed")
    guard_sha = _write_new(root_path, RESOURCE_GUARD_RELATIVE_PATH, guard_payload)
    authorization_payload = build_execution_authorization_payload_v11(
        root_path,
        execution_contract_binding=_binding(root_path, EXECUTION_CONTRACT_RELATIVE_PATH),
        source_closure_binding=_binding(root_path, SOURCE_CLOSURE_RELATIVE_PATH),
        runtime_lock_binding=_binding(root_path, RUNTIME_LOCK_RELATIVE_PATH),
    ...
    if (datetime.now(UTC) - started).total_seconds() > MAXIMUM_FRESH_AUTHORIZATION_CHAIN_SECONDS:
        raise FreshAuthorizationV9Error("v11 dynamic authorization chain exceeded its launch budget")
    ...
    return FreshLaunchAuthorizationV11(authorization, guard_sha, created_at)
...
__all__ = [
    ...
    "create_fresh_launch_authorization_v11",
]
