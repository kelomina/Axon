"""Fail-closed authorization for one contained Loop167 Phase-B v12 execution."""
...
from .child_attestation_v12 import verify_child_job_attestation_v12
...
from .resource_guard_v12 import RESOURCE_GUARD_SCHEMA, verify_resource_guard_v12
from .supervisor_v12 import validate_launch_receipt_v12

AUTHORIZATION_SCHEMA = "axon_loop167_phase_b_run_authorization_v12"
...
class VerifiedExecutionAuthorizationV12:
    project_root: Path
...
def _binding(root: Path, value: object, *, label: str, expected_path: str, expected_schema: str | None = None) -> tuple[dict[str, str], Mapping[str, Any] | None]:
    if not isinstance(value, Mapping) or set(value) != {"path", "sha256"} or value.get("path") != expected_path:
        raise PhaseBContractError(f"v12 authorization {label} binding drifted")
    ...
    if expected_schema is not None and (payload.get("schema") != expected_schema or payload.get("loop_id") != LOOP_ID):
        raise PhaseBContractError(f"v12 authorization {label} schema drifted")
...
def _validate_payload(
    ...
    *,
    ...
    phase: str,
    launch_id: str | None,
) -> VerifiedExecutionAuthorizationV12:
    if phase not in {"prelaunch", "contained_child", "attested_child", "leased_child_pre_raw"}:
        raise PhaseBContractError("v12 authorization phase is invalid")
    expected_keys = {
    ...
    contract = verify_execution_contract_v12(root, contract_binding)
    ...
    if protocol_binding != dict(contract.protocol_binding) or protocol.get("claim_scope") != "local_train_only_structural_delta_diagnostic_not_model_quality_promotion_or_full_test":
        raise PhaseBContractError("v12 authorization protocol binding drifted")
    ...
    if runtime_payload.get("controller") != controller_binding or runtime_payload.get("supervisor") != supervisor_binding or runtime_payload.get("execution_contract") != contract_binding:
        raise PhaseBContractError("v12 authorization runtime lock bindings drifted")
    ...
    verified_guard = verify_resource_guard_v12(
        root,
    ...
    if payload.get("output_catalog") != [dict(entry) for entry in FIXED_OUTPUT_CATALOG] or payload.get("output_catalog_sha256") != output_catalog_sha256():
        raise PhaseBContractError("v12 authorization output catalog drifted")
    if payload.get("lease") != EXPECTED_LEASE or payload.get("ready_for") != AUTHORIZED_READY_FOR:
        raise PhaseBContractError("v12 authorization lease or ready state drifted")
    if payload.get("forbidden") != list(EXPECTED_FORBIDDEN):
        raise PhaseBContractError("v12 authorization forbidden scope drifted")
    if payload.get("raw_open_attempts") != 0:
        raise PhaseBContractError("v12 authorization records raw access")
    _parse_utc(payload.get("created_at_utc"), label="authorization.created_at_utc")
    if phase == "prelaunch":
        output_paths = assert_output_catalog_is_fresh_v12(root, payload["output_catalog"])
    else:
        if phase == "contained_child":
            output_paths = assert_contained_child_prelease_surface_v12(root, payload["output_catalog"])
        elif phase == "attested_child":
            output_paths = assert_attested_child_prelease_surface_v12(root, payload["output_catalog"])
        else:
            output_paths = assert_leased_child_pre_raw_surface_v12(root, payload["output_catalog"])
        if not isinstance(launch_id, str) or not launch_id:
            raise PhaseBContractError("v12 attested child authorization requires launch id")
        ...
        launch = validate_launch_receipt_v12(
            root,
            launch_path,
            mode="execute",
        ...
        if phase in {"attested_child", "leased_child_pre_raw"}:
            verify_child_job_attestation_v12(
                root,
    ...
    return VerifiedExecutionAuthorizationV12(
        project_root=root,
        authorization_path=Path(),
...
def build_execution_authorization_payload_v12(
    root: Path | str,
    *,
    execution_contract_binding: Mapping[str, str],
    source_closure_binding: Mapping[str, str],
...
def validate_execution_authorization_v12(
    root: Path | str,
    ...
    *,
    ...
    if safe_project_relative_path(root_path, authorization_path, require_exists=True, require_regular_file=True) != RUN_AUTHORIZATION_RELATIVE_PATH:
        raise PhaseBContractError("v12 authorization path drifted")
    ...
    if sha256_file(expected_path) != digest:
        raise PhaseBContractError("v12 authorization changed during validation")
    return VerifiedExecutionAuthorizationV12(
...
__all__ = [
    ...
    "VerifiedExecutionAuthorizationV12",
    "build_execution_authorization_payload_v12",
