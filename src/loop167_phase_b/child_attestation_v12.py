"""Child-side containment attestation written before the v12 authorization lease."""
...
from .supervisor_v12 import _write_new_json, validate_launch_receipt_v12
...
def _verify_live_process_identity(pid: int, expected_creation_time_filetime: int) -> dict[str, int | bool | str]:
    if os.name != "nt" or pid <= 0 or expected_creation_time_filetime <= 0:
        raise PhaseBContractError("v12 supervisor identity input is invalid")
    ...
    if not handle:
        raise PhaseBContractError(f"v12 supervisor identity cannot open PID {pid}: Win32 {ctypes.get_last_error()}")
    try:
        if kernel32.WaitForSingleObject(handle, 0) != 0x00000102:
            raise PhaseBContractError("v12 supervisor is no longer active")
        creation = FileTime()
        exit_time = FileTime()
        ...
        if not kernel32.GetProcessTimes(
            ...
            raise PhaseBContractError(
                f"v12 supervisor identity cannot read creation time: Win32 {ctypes.get_last_error()}"
            )
        ...
        if observed_creation != expected_creation_time_filetime:
            raise PhaseBContractError("v12 supervisor PID creation time drifted")
        return {
            "pid": pid,
...
def build_child_job_attestation_payload_v12(
    root: Path | str,
    *,
    ...
    validated_launch = validate_launch_receipt_v12(
        ...
        expected_pid=None,
        expected_supervisor_pid=int(os.environ.get("AXON_LOOP167_V12_SUPERVISOR_PID", "0") or "0"),
    )
    ...
    if job_proof_binding["path"] != "src/loop166/windows_job.py":
        raise PhaseBContractError("v12 Loop166 Job proof binding path drifted")
    if lineage_proof_binding["path"] != "src/loop166/windows_process_lineage.py":
        raise PhaseBContractError("v12 Loop166 lineage proof binding path drifted")
    ...
    try:
        ...
        raise PhaseBContractError("v12 child cannot prove current Job membership") from error
    ...
    try:
        ...
        raise PhaseBContractError("v12 child cannot prove launcher lineage and liveness") from error
    ...
    if int(assignment["process_pid"]) != expected_launcher_pid:
        raise PhaseBContractError("v12 pre-resume receipt launcher identity differs from bounded lineage")
...
def write_child_job_attestation_v12(root: Path | str, payload: Mapping[str, Any]) -> tuple[Path, str]:
    root_path = safe_project_root(root)
    assert_contained_child_prelease_surface_v12(root_path)
    output_path = _output_path(root_path)
    digest = _write_new_json(root_path, output_path, payload)
    return output_path, digest


def verify_child_job_attestation_v12(
    root: Path | str,
    *,
    ...
    if not path.is_file() or path.is_symlink():
        raise PhaseBContractError("v12 child Job attestation is unavailable")
    try:
        raw = path.read_bytes()
        payload = json.loads(raw.decode("utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise PhaseBContractError("v12 child Job attestation is unavailable") from error
    if not isinstance(payload, dict) or canonical_json_bytes(payload) != raw:
        raise PhaseBContractError("v12 child Job attestation is not canonical")
    expected = build_child_job_attestation_payload_v12(
        root_path,
    ...
    if payload != expected:
        raise PhaseBContractError("v12 child Job attestation drifted")
