"""Pinned runtime facts for a contained Loop167 Phase-B v7 controller."""
...
from .invocation_v7 import (
    THREAD_ENVIRONMENT_V7,
    _loaded_external_runtime_modules,
    canonical_argv_hashes_v7,
    canonical_argv_v7,
    validate_current_runtime_invocation_v7,
    validate_runtime_envelope_v7,
    validate_thread_environment_v7,
)
...
def _clean_external_import_state() -> None:
    ...
    if loaded:
        raise PhaseBContractError("v7 runtime lock checked after external imports: " + ", ".join(loaded))
...
def _python_binding(root: Path) -> dict[str, str]:
    ...
    if relative != "vnev/Scripts/python.exe":
        raise PhaseBContractError("v7 runtime Python is outside vnev")
    return {
...
def _module_binding(root: Path, distribution: str, module_name: str) -> dict[str, str]:
    ...
    if not isinstance(module_file, str):
        raise PhaseBContractError(f"v7 runtime module lacks a file: {module_name}")
    ...
    if not relative.startswith("vnev/"):
        raise PhaseBContractError(f"v7 runtime module is outside vnev: {module_name}")
    return {
...
def _source_binding(root: Path, binding: Mapping[str, str], *, expected_path: str, label: str) -> dict[str, str]:
    ...
    if relative != expected_path:
        raise PhaseBContractError(f"v7 runtime lock {label} path drifted")
    return {"path": relative, "sha256": digest}


def build_runtime_lock_payload_v7(
    root: Path | str,
    *,
    ...
    validate_runtime_envelope_v7(root_path)
    ...
    return {
        ...
        "canonical_argv": {
            f"{role}_{mode}": list(canonical_argv_v7(role, mode))
            for role in ("supervisor", "controller")
            for mode in ("preflight", "execute")
        },
        "canonical_argv_sha256": canonical_argv_hashes_v7(),
        "thread_environment": dict(THREAD_ENVIRONMENT_V7),
...
def validate_runtime_lock_v7(
    ...
    *,
    ...
    if set(payload) != expected_keys or payload.get("schema") != RUNTIME_LOCK_SCHEMA:
        raise PhaseBContractError("v7 runtime lock schema drifted")
    ...
    if payload.get("thread_environment") != THREAD_ENVIRONMENT_V7:
        raise PhaseBContractError("v7 runtime lock thread environment drifted")
    if payload.get("isolated_python_required") is not True:
        raise PhaseBContractError("v7 runtime lock must require isolated Python")
    if payload.get("network_fetch_allowed") is not False or payload.get("dependency_install_allowed") is not False:
        raise PhaseBContractError("v7 runtime lock permits unsafe runtime actions")
    if verify_current_runtime:
        _clean_external_import_state()
        validate_current_runtime_invocation_v7(root_path, role=role, mode=mode)
    else:
        validate_thread_environment_v7()
    ...
    if payload.get("controller") != expected_controller or payload.get("supervisor") != expected_supervisor:
        raise PhaseBContractError("v7 runtime lock source binding drifted")
    ...
    expected_argv = {
        f"{role_name}_{mode_name}": list(canonical_argv_v7(role_name, mode_name))
        for role_name in ("supervisor", "controller")
        for mode_name in ("preflight", "execute")
    }
    if payload.get("canonical_argv") != expected_argv or payload.get("canonical_argv_sha256") != canonical_argv_hashes_v7():
        raise PhaseBContractError("v7 runtime lock argv drifted")
    if payload.get("python") != _python_binding(root_path):
        raise PhaseBContractError("v7 runtime lock Python binding drifted")
    ...
    if payload.get("packages") != expected_packages:
        raise PhaseBContractError("v7 runtime lock package binding drifted")
    return VerifiedRuntimeLockV7(
        role=role,
        mode=mode,
