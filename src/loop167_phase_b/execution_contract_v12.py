"""Independent v12 authority contract for the user-authorized no-prelaunch-memory-floor remediation."""
...
ARTIFACT_DIRECTORY = f"{PARENT_ARTIFACT_DIRECTORY}/phase_b_v12_dual_identity_job_attestation_remediation"
REPORT_DIRECTORY = "reports/roadmap_9997/loop167/phase_b_v12_dual_identity_job_attestation_remediation"
...
EXECUTION_CONTRACT_RELATIVE_PATH = f"{ARTIFACT_DIRECTORY}/phase_b_execution_contract_v12.json"
SOURCE_CLOSURE_RELATIVE_PATH = f"{ARTIFACT_DIRECTORY}/phase_b_source_closure_v12.json"
RUNTIME_LOCK_RELATIVE_PATH = f"{ARTIFACT_DIRECTORY}/phase_b_runtime_lock_v12.json"
RESOURCE_GUARD_RELATIVE_PATH = f"{ARTIFACT_DIRECTORY}/phase_b_resource_guard_v12.json"
RUN_AUTHORIZATION_RELATIVE_PATH = f"{ARTIFACT_DIRECTORY}/phase_b_run_authorization_v12.json"

SUPERVISOR_RELATIVE_PATH = "scripts/run_loop167_phase_b_supervisor_v12.py"
CONTROLLER_RELATIVE_PATH = "scripts/run_loop167_phase_b_controller_v12.py"
...
PARENT_V10_PRELEASE_ATTESTATION_SCHEMA = "axon_loop167_phase_b_v10_prelease_no_dynamic_execution_attestation_v12"
EXECUTION_CONTRACT_SCHEMA = "axon_loop167_phase_b_execution_contract_v12"
CLAIM_SCOPE = "v12_user_authorized_no_prelaunch_memory_floor_train_only_fixed_oof_not_promotion_or_heldout_evaluation"
AUTHORIZATION_CLAIM_SCOPE = "single_v12_user_authorized_no_prelaunch_memory_floor_train_only_raw_pass_then_fixed_oof"
...
EXPECTED_LEASE = {
    "lease_id": "loop167-phase-b-v12-windows-job-abi-remediation-train-oof-v1",
    "marker_path": f"{REPORT_DIRECTORY}/phase_b_execution_consumed_v12.json",
    "consume_before_first_raw_open": True,
    "failed_attempt_consumes_lease": True,
    "retry_resume_or_rescan_allowed": False,
}
...
FIXED_OUTPUT_CATALOG: tuple[dict[str, str], ...] = (
    {
        ...
        "path": f"{REPORT_DIRECTORY}/phase_b_supervisor_launch_v12.json",
        "kind": "pre_resume_containment_receipt",
    },
    {
        "name": "supervisor_exit_receipt",
        "path": f"{REPORT_DIRECTORY}/phase_b_supervisor_exit_v12.json",
        "kind": "supervisor_exit_receipt",
    },
    {
        "name": "supervisor_failure_receipt",
        "path": f"{REPORT_DIRECTORY}/phase_b_supervisor_failure_v12.json",
        "kind": "pre_resume_failure_receipt",
    },
    {
        "name": "child_job_attestation",
        "path": f"{REPORT_DIRECTORY}/phase_b_child_job_attestation_v12.json",
        "kind": "child_membership_receipt",
    },
    {
        "name": "feature_cache",
        "path": f"{REPORT_DIRECTORY}/phase_b_feature_cache_v12.npz",
        "kind": "numeric_feature_cache",
    },
    {
        "name": "raw_progress_ledger",
        "path": f"{REPORT_DIRECTORY}/phase_b_raw_progress_v12.jsonl",
        "kind": "append_only_raw_ledger",
    },
    {
        "name": "fit_progress_ledger",
        "path": f"{REPORT_DIRECTORY}/phase_b_fit_progress_v12.jsonl",
        "kind": "append_only_fit_ledger",
    },
    {
        "name": "controller_failure_receipt",
        "path": f"{REPORT_DIRECTORY}/phase_b_controller_failure_v12.json",
        "kind": "post_lease_controller_failure_receipt",
    },
    {
        "name": "execution_receipt",
        "path": f"{REPORT_DIRECTORY}/phase_b_execution_receipt_v12.json",
...
class VerifiedParentV10PreleaseAttestationV12:
...
@dataclass(frozen=True)
class VerifiedExecutionContractV12:
    contract_path: Path
    contract_sha256: str
...
def _fixed_binding(value: object, *, label: str, expected_path: str) -> dict[str, str]:
    ...
    if path != expected_path or not isinstance(digest, str) or len(digest) != 64:
        raise PhaseBContractError(f"{label} binding drifted from its fixed v12 path")
...
def ensure_v12_static_artifact_parent(root: Path | str, relative_path: str) -> Path:
    ...
    if not canonical.startswith(f"{ARTIFACT_DIRECTORY}/"):
        raise PhaseBContractError("v12 static artifact is outside its fixed root")
    cursor = root_path
    for component in canonical.split("/")[:-1]:
        ...
        try:
            ...
            raise PhaseBContractError("v12 static artifact parent is unavailable") from error
        attributes = int(getattr(stat_result, "st_file_attributes", 0))
        if stat.S_ISLNK(stat_result.st_mode) or bool(attributes & 0x0400) or not stat.S_ISDIR(stat_result.st_mode):
            raise PhaseBContractError("v12 static artifact parent is unsafe")
...
def build_parent_v10_prelease_attestation_payload_v12(root: Path | str) -> dict[str, Any]:
    ...
    return {
        ...
        "replacement_scope": "one_new_v12_redirector_lineage_attestation_remediation_chain_only",
    }


def validate_parent_v10_prelease_attestation_payload_v12(root: Path, payload: Mapping[str, Any]) -> None:
    expected_keys = {
        "schema",
        "loop_id",
        "status",
    ...
    if set(payload) != expected_keys or payload.get("schema") != PARENT_V10_PRELEASE_ATTESTATION_SCHEMA:
        raise PhaseBContractError("v12 parent v10 attestation schema drifted")
    if (
        ...
        raise PhaseBContractError("v12 parent v10 attestation identity drifted")
    expected = build_parent_v10_prelease_attestation_payload_v12(root)
    if dict(payload) != expected:
        raise PhaseBContractError("v12 parent v10 pre-execution attestation facts drifted")


def verify_parent_v10_prelease_attestation_v12(
    root: Path | str,
    binding: Mapping[str, str],
) -> VerifiedParentV10PreleaseAttestationV12:
    root_path = safe_project_root(root)
    ...
    payload = require_canonical_json(path)
    validate_parent_v10_prelease_attestation_payload_v12(root_path, payload)
    return VerifiedParentV10PreleaseAttestationV12(
        attestation_path=path,
...
def _validated_output_catalog(catalog: object) -> tuple[Mapping[str, str], ...]:
    ...
    if materialized != [dict(entry) for entry in FIXED_OUTPUT_CATALOG]:
        raise PhaseBContractError("v12 output catalog drifted")
    return tuple(MappingProxyType(dict(entry)) for entry in FIXED_OUTPUT_CATALOG)


def resolve_output_catalog_v12(root: Path | str, catalog: object = FIXED_OUTPUT_CATALOG) -> Mapping[str, Path]:
    root_path = safe_project_root(root)
    paths: dict[str, Path] = {}
    for entry in _validated_output_catalog(catalog):
        ...
        if name in paths:
            raise PhaseBContractError("v12 output catalog repeats a name")
        paths[name] = path
    return MappingProxyType(paths)


def assert_output_catalog_is_fresh_v12(root: Path | str, catalog: object = FIXED_OUTPUT_CATALOG) -> Mapping[str, Path]:
    paths = resolve_output_catalog_v12(root, catalog)
    for path in paths.values():
        if path.exists() or path.is_symlink():
            raise PhaseBContractError("v12 output already exists or is unsafe")
    marker = safe_project_path(root, EXPECTED_LEASE["marker_path"], require_exists=False)
    if marker.exists() or marker.is_symlink():
        raise PhaseBContractError("v12 execution lease already exists or is unsafe")
    return paths


def assert_contained_child_prelease_surface_v12(
    root: Path | str,
    ...
    if not launch_path.is_file() or launch_path.is_symlink():
        raise PhaseBContractError("v12 contained child requires the pre-resume launch receipt")
    for name, path in paths.items():
        if name == "supervisor_launch_receipt":
            continue
        if path.exists() or path.is_symlink():
            raise PhaseBContractError("v12 contained child prelease output surface drifted")
    marker = safe_project_path(root, EXPECTED_LEASE["marker_path"], require_exists=False)
    if marker.exists() or marker.is_symlink():
        raise PhaseBContractError("v12 contained child execution lease already exists or is unsafe")
    return paths


def assert_attested_child_prelease_surface_v12(
    root: Path | str,
    catalog: object = FIXED_OUTPUT_CATALOG,
) -> Mapping[str, Path]:
    paths = resolve_output_catalog_v12(root, catalog)
    for name in ("supervisor_launch_receipt", "child_job_attestation"):
        ...
        if not path.is_file() or path.is_symlink():
            raise PhaseBContractError("v12 attested child requires its containment receipts")
    for name, path in paths.items():
        if name in {"supervisor_launch_receipt", "child_job_attestation"}:
            continue
        if path.exists() or path.is_symlink():
            raise PhaseBContractError("v12 attested child prelease output surface drifted")
    marker = safe_project_path(root, EXPECTED_LEASE["marker_path"], require_exists=False)
    if marker.exists() or marker.is_symlink():
        raise PhaseBContractError("v12 attested child execution lease already exists or is unsafe")
    return paths


def assert_leased_child_pre_raw_surface_v12(
    root: Path | str,
    catalog: object = FIXED_OUTPUT_CATALOG,
) -> Mapping[str, Path]:
    ...
    for name in ("supervisor_launch_receipt", "child_job_attestation"):
        path = paths[name]
        if not path.is_file() or path.is_symlink():
            raise PhaseBContractError("v12 leased child requires its containment receipts")
    for name, path in paths.items():
        if name in {"supervisor_launch_receipt", "child_job_attestation"}:
            continue
        if path.exists() or path.is_symlink():
            raise PhaseBContractError("v12 leased child pre-raw output surface drifted")
...
def build_execution_contract_payload_v12(
    root: Path | str,
    *,
    parent_v10_prelease_attestation_binding: Mapping[str, str],
) -> dict[str, Any]:
    root_path = safe_project_root(root)
    attestation = verify_parent_v10_prelease_attestation_v12(root_path, parent_v10_prelease_attestation_binding)
    ...
    assert_output_catalog_is_fresh_v12(root_path)
    return {
...
def validate_execution_contract_payload_v12(root: Path, payload: Mapping[str, Any]) -> None:
    expected_keys = {
        "schema",
        "loop_id",
        "phase_b_protocol",
    ...
    if set(payload) != expected_keys or payload.get("schema") != EXECUTION_CONTRACT_SCHEMA:
        raise PhaseBContractError("v12 execution contract schema drifted")
    if payload.get("loop_id") != LOOP_ID or payload.get("claim_scope") != CLAIM_SCOPE:
        raise PhaseBContractError("v12 execution contract identity drifted")
    _require_json_binding(
        root,
    ...
    if payload.get("windows_job_boundary") != {
        ...
        raise PhaseBContractError("v12 Windows Job boundary drifted")
    if payload.get("prelaunch_available_memory_floor") != {
        "enforced": False,
        "authority": "explicit_user_authorization",
        "job_process_memory_cap_bytes": EXPECTED_RESOURCE_CONTRACT["maximum_training_peak_rss_bytes"],
    }:
        raise PhaseBContractError("v12 user-authorized memory-floor contract drifted")
    if payload.get("resource_contract") != EXPECTED_RESOURCE_CONTRACT:
        raise PhaseBContractError("v12 resource contract drifted")
    if payload.get("b1_sampling_indicators") != B1_SAMPLING_INDICATORS_CONTRACT:
        raise PhaseBContractError("v12 B1 sampling contract drifted")
    if payload.get("raw_root_relative") != RAW_ROOT_RELATIVE_PATH:
        raise PhaseBContractError("v12 raw root drifted")
    if canonical_project_relative_path(payload["raw_root_relative"]) != RAW_ROOT_RELATIVE_PATH:
        raise PhaseBContractError("v12 raw root is not canonical")
    _validated_output_catalog(payload.get("output_catalog"))
    if payload.get("output_catalog_sha256") != output_catalog_sha256():
        raise PhaseBContractError("v12 output catalog hash drifted")
    if payload.get("lease") != EXPECTED_LEASE or payload.get("forbidden") != EXPECTED_FORBIDDEN:
        raise PhaseBContractError("v12 lease or forbidden scope drifted")
    if payload.get("execution_authorization_required") is not True or payload.get("raw_open_attempts_before_lease") != 0:
        raise PhaseBContractError("v12 prelease contract drifted")
    resolve_output_catalog_v12(root, payload["output_catalog"])


def verify_execution_contract_v12(root: Path | str, binding: Mapping[str, str]) -> VerifiedExecutionContractV12:
    root_path = safe_project_root(root)
    ...
    payload = require_canonical_json(path)
    validate_execution_contract_payload_v12(root_path, payload)
    return VerifiedExecutionContractV12(
        contract_path=path,
        ...
        output_paths=resolve_output_catalog_v12(root_path, payload["output_catalog"]),
        lease=MappingProxyType(dict(EXPECTED_LEASE)),
    )


def canonical_controller_execute_argv_v12(argv: Sequence[str]) -> tuple[str, ...]:
    normalized = tuple(argv)
    if normalized != CANONICAL_CONTROLLER_EXECUTE_ARGV:
        raise PhaseBContractError("v12 controller execute argv is not sealed")
    return normalized
...
__all__ = [
    ...
    "SUPERVISOR_RELATIVE_PATH",
    "VNEV_PYTHON_RELATIVE_PATH",
    "VerifiedExecutionContractV12",
    "assert_attested_child_prelease_surface_v12",
    "assert_contained_child_prelease_surface_v12",
    "assert_leased_child_pre_raw_surface_v12",
    "assert_output_catalog_is_fresh_v12",
    "build_execution_contract_payload_v12",
    "build_parent_v10_prelease_attestation_payload_v12",
    "canonical_controller_execute_argv_v12",
    "ensure_v12_static_artifact_parent",
    "output_catalog_sha256",
    "resolve_output_catalog_v12",
    "validate_execution_contract_payload_v12",
    "validate_parent_v10_prelease_attestation_payload_v12",
    "verify_execution_contract_v12",
    "verify_parent_v10_prelease_attestation_v12",
]
