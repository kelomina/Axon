"""Raw-free static validation for the Loop167 Phase-B v8 control plane."""
...
V8_SCOPE = "v8_in_process_fresh_authorization_remediation_single_authorized_train_only_execution_no_heldout_access"
...
def _verify_source_files(root: Path, source_files: object) -> None:
    if not isinstance(source_files, list) or not source_files:
        raise PhaseBContractError("v8 source closure has no source-file bindings")
    ...
    for binding in source_files:
        ...
        if relative in observed:
            raise PhaseBContractError("v8 source closure repeats a source file")
        observed.add(relative)
    if not REQUIRED_SOURCE_PATHS.issubset(observed):
        raise PhaseBContractError("v8 source closure omits required control-plane sources")
...
def _validate_source_closure(
    ...
    *,
    ...
    if set(payload) != expected_keys or payload.get("schema") != SOURCE_CLOSURE_SCHEMA:
        raise PhaseBContractError("v8 source closure schema drifted")
    if payload.get("loop_id") != LOOP_ID or payload.get("scope") != V8_SCOPE:
        raise PhaseBContractError("v8 source closure identity drifted")
    if payload.get("static_preflight_ready") is not True or payload.get("phase_b_raw_execution_ready") is not False:
        raise PhaseBContractError("v8 source closure readiness drifted")
    if payload.get("dynamic_execution_gates") != EXPECTED_DYNAMIC_GATES:
        raise PhaseBContractError("v8 source closure dynamic gates drifted")
    if payload.get("remaining_execution_blockers") != EXPECTED_BLOCKERS:
        raise PhaseBContractError("v8 source closure blockers drifted")
    _binding(payload.get("parent_v7_prelease_attestation"), label="parent_attestation", expected_path=PARENT_V7_PRELEASE_ATTESTATION_RELATIVE_PATH)
    ...
    if _binding(payload.get("controller"), label="controller", expected_path=CONTROLLER_RELATIVE_PATH) != dict(controller_binding):
        raise PhaseBContractError("v8 source closure controller binding drifted")
    if _binding(payload.get("supervisor"), label="supervisor", expected_path=SUPERVISOR_RELATIVE_PATH) != dict(supervisor_binding):
        raise PhaseBContractError("v8 source closure supervisor binding drifted")
...
def _source_file_binding(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise PhaseBContractError(f"v8 source closure lacks a unique {label} binding")
    _path, digest = verify_safe_file_binding(root, candidates[0], label=label)
    return {"path": expected_path, "sha256": digest}


def validate_static_preflight_v8(
    root: Path | str,
    *,
    source_closure_binding: Mapping[str, str],
    controller_binding: Mapping[str, str],
    ...
    if phase not in {"prelaunch", "contained_child", "attested_child", "leased_child_pre_raw"}:
        raise PhaseBContractError("v8 static preflight phase is invalid")
    ...
    if sha256_file(_controller_path) != normalized_controller["sha256"]:
        raise PhaseBContractError("v8 controller changed during static preflight")
    if sha256_file(_supervisor_path) != normalized_supervisor["sha256"]:
        raise PhaseBContractError("v8 supervisor changed during static preflight")
    ...
    verify_parent_v7_prelease_attestation_v8(root_path, closure["parent_v7_prelease_attestation"])
    contract = verify_execution_contract_v8(root_path, closure["phase_b_execution_contract"])
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v8" or runtime_lock.get("loop_id") != LOOP_ID:
        raise PhaseBContractError("v8 runtime lock identity drifted")
    if runtime_lock.get("controller") != normalized_controller or runtime_lock.get("supervisor") != normalized_supervisor:
        raise PhaseBContractError("v8 runtime lock controller bindings drifted")
    if runtime_lock.get("execution_contract") != dict(closure["phase_b_execution_contract"]):
        raise PhaseBContractError("v8 runtime lock execution contract binding drifted")
    if phase == "prelaunch":
        assert_output_catalog_is_fresh_v8(root_path, contract.output_catalog)
    elif phase == "contained_child":
        assert_contained_child_prelease_surface_v8(root_path, contract.output_catalog)
    elif phase == "attested_child":
        assert_attested_child_prelease_surface_v8(root_path, contract.output_catalog)
    else:
        assert_leased_child_pre_raw_surface_v8(root_path, contract.output_catalog)
