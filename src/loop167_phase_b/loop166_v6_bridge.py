...
class JobMembershipV6Error(RuntimeError):
    """The sealed Loop166 Job membership proof could not be completed."""


class ProcessLineageV6Error(RuntimeError):
...
def _load_loop166_module(
    ...
    path, observed_sha256 = verify_safe_file_binding(
        ...
        label="v6_loop166_proof",
    )
    module_name = f"_axon_loop167_v6_{path.stem}_{observed_sha256[:16]}"
    specification = importlib.util.spec_from_file_location(module_name, path)
    if specification is None or specification.loader is None:
        raise RuntimeError(f"Loop166 v6 bridge cannot load: {relative_path}")
    ...
    if sha256_file(path) != observed_sha256:
        sys.modules.pop(module_name, None)
        raise RuntimeError(f"Loop166 v6 bridge source changed while loading: {relative_path}")
    return module
...
def _proof_module(
    ...
    *,
    ...
    if dict(binding).get("path") != expected_path or set(binding) != {"path", "sha256"}:
        raise RuntimeError(f"Loop166 v6 bridge binding drifted: {expected_path}")
...
def audit_current_process_job_membership(
    ...
    *,
    ...
    try:
        ...
        raise JobMembershipV6Error("v6 current child Job membership proof failed") from error
...
def audit_process_job_membership(
    ...
    try:
        ...
        raise JobMembershipV6Error("v6 launcher Job membership proof failed") from error
...
def validate_spawn_lineage(
    ...
    *,
    ...
    try:
        ...
        raise ProcessLineageV6Error("v6 child process lineage proof failed") from error
