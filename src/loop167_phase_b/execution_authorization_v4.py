"""Fail-closed run authorization for the sealed Loop167 Phase-B v4 execution."""
...
from .execution_contract_v4 import (
    ...
    assert_output_catalog_is_fresh_v4,
    canonical_execute_argv_v4,
    output_catalog_sha256,
    verify_execution_contract_v4,
)
from .invocation_v4 import canonical_argv_hashes_v4
...
from .resource_guard_v4 import RESOURCE_GUARD_SCHEMA, verify_resource_guard_v4

AUTHORIZATION_SCHEMA = "axon_loop167_phase_b_run_authorization_v4"
AUTHORIZATION_STATUS = "authorized_pending_one_shot_lease"
SOURCE_CLOSURE_SCHEMA = "axon_loop167_phase_b_source_closure_v4"
RUNTIME_LOCK_SCHEMA = "axon_loop167_phase_b_runtime_lock_v4"
...
class VerifiedExecutionAuthorizationV4:
...
def _validate_runtime_lock_payload(
    ...
    *,
    ...
    if payload.get("canonical_argv_sha256") != canonical_argv_hashes_v4():
    ...
    if payload.get("network_fetch_allowed") is not False or payload.get("dependency_install_allowed") is not False:
        raise PhaseBContractError("Runtime lock v4 permits an unsafe dependency action")
...
def _validate_authorization_payload(
    ...
    *,
    now_utc: datetime,
) -> VerifiedExecutionAuthorizationV4:
    expected_keys = {
        "schema",
        "loop_id",
        "claim_scope",
    ...
    if set(payload) != expected_keys or payload.get("schema") != AUTHORIZATION_SCHEMA:
        raise PhaseBContractError("Execution authorization v4 schema drifted")
    if payload.get("loop_id") != LOOP_ID or payload.get("claim_scope") != AUTHORIZATION_CLAIM_SCOPE:
        raise PhaseBContractError("Execution authorization v4 identity or scope drifted")
    if (
        payload.get("status") != AUTHORIZATION_STATUS
        or payload.get("execution_authorization_granted") is not True
    ):
        raise PhaseBContractError("Execution authorization v4 is not granted")
    ...
    argv = canonical_execute_argv_v4(payload.get("canonical_execute_argv", ()))
    ...
    verified_guard = verify_resource_guard_v4(
        root,
    ...
    output_paths = assert_output_catalog_is_fresh_v4(root, payload["output_catalog"])
    ...
    return VerifiedExecutionAuthorizationV4(
...
def build_execution_authorization_payload_v4(
    root: Path | str,
    *,
    execution_contract_binding: Mapping[str, str],
    source_closure_binding: Mapping[str, str],
    ...
    contract = verify_execution_contract_v4(root_path, contract_binding)
    contract_payload = require_canonical_json(contract_path)
...
def validate_execution_authorization_v4(
    root: Path | str,
    authorization_path: Path | str,
    *,
    now_utc: datetime,
) -> VerifiedExecutionAuthorizationV4:
    """Validate the fixed authorization path and every bound v4 prerequisite."""
