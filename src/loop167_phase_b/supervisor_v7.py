"""Raw-free suspended-child supervision for the Loop167 Phase-B v7 route."""
...
from .windows_job_v7 import WindowsJobV7, WindowsJobV7Error

LAUNCH_RECEIPT_SCHEMA = "axon_loop167_phase_b_v7_pre_resume_launch_receipt"
EXIT_RECEIPT_SCHEMA = "axon_loop167_phase_b_v7_supervisor_exit_receipt"
FAILURE_RECEIPT_SCHEMA = "axon_loop167_phase_b_v7_supervisor_failure_receipt"
MAXIMUM_TIMEOUT_SECONDS = 28_800


class SupervisorV7Error(PhaseBContractError):
    """The raw-free controller supervisor cannot prove containment."""


@dataclass(frozen=True)
class SupervisorConfigV7:
    project_root: Path
    mode: str
    command: tuple[str, ...]
    launch_receipt: Path
...
@dataclass(frozen=True)
class ValidatedLaunchReceiptV7:
...
def _current_process_identity() -> dict[str, int | str]:
    if os.name != "nt":
        raise SupervisorV7Error("v7 supervisor identity is available only on Windows")
    import ctypes
    ...
    if not kernel32.GetProcessTimes(
        ...
        raise SupervisorV7Error(f"Supervisor GetProcessTimes failed: Win32 {ctypes.get_last_error()}")
    return {
        "pid": os.getpid(),
...
def _safe_output_path(root: Path, path: Path) -> Path:
    try:
        ...
        raise SupervisorV7Error("Supervisor output is outside the project root") from error
    cursor = root_path
    for component in relative_path.parts[:-1]:
        ...
        try:
            ...
            raise SupervisorV7Error("Supervisor output parent is unavailable") from error
        if _is_link_or_reparse(stat_result) or not stat.S_ISDIR(stat_result.st_mode):
            raise SupervisorV7Error("Supervisor output parent is unsafe")
...
def _fsync_parent_directory(parent: Path) -> None:
    ...
    try:
        ...
        raise SupervisorV7Error("Supervisor receipt parent cannot be opened for durable sync") from error
    try:
        os.fsync(descriptor)
    except OSError as error:
        raise SupervisorV7Error("Supervisor receipt parent durable sync failed") from error
    finally:
        os.close(descriptor)
...
def _write_new_json(root: Path, path: Path, payload: Mapping[str, Any]) -> str:
    ...
    try:
        ...
        raise SupervisorV7Error(f"Supervisor output already exists: {output_path}") from error
    except OSError as error:
        raise SupervisorV7Error("Supervisor receipt cannot be created") from error
...
def _require_binding(value: object, *, name: str) -> dict[str, str]:
    if not isinstance(value, Mapping) or set(value) != {"path", "sha256"}:
        raise SupervisorV7Error(f"Supervisor static binding {name} is invalid")
    path = value.get("path")
...
def _validate_config(config: SupervisorConfigV7) -> tuple[Path, dict[str, dict[str, str]]]:
    if config.mode not in {"preflight", "execute"}:
        raise SupervisorV7Error("Supervisor mode must be preflight or execute")
    if not config.command or any(not isinstance(item, str) or not item for item in config.command):
        raise SupervisorV7Error("Supervisor command is invalid")
    if not isinstance(config.timeout_seconds, int) or not 0 < config.timeout_seconds <= MAXIMUM_TIMEOUT_SECONDS:
        raise SupervisorV7Error("Supervisor timeout is invalid")
    if not isinstance(config.memory_limit_bytes, int) or config.memory_limit_bytes <= 0:
        raise SupervisorV7Error("Supervisor memory limit is invalid")
    try:
        root = config.project_root.resolve(strict=True)
        executable = Path(config.command[0]).resolve(strict=True)
        executable.relative_to(root)
    except (OSError, ValueError) as error:
        raise SupervisorV7Error("Supervisor executable is unavailable or outside the project") from error
    if Path.cwd().resolve(strict=True) != root:
        raise SupervisorV7Error("Supervisor must run from the canonical project root")
    bindings = {name: _require_binding(value, name=name) for name, value in config.static_bindings.items()}
    if not bindings:
        raise SupervisorV7Error("Supervisor requires static source bindings")
    ...
    if controller_binding is not None:
        if len(config.command) != 4 or config.command[1] != "-I" or config.command[3] not in {"--preflight", "--execute"}:
            raise SupervisorV7Error("Supervisor controller command shape is invalid")
        ...
        if config.command[2] != controller_path or Path(config.command[2]).is_absolute() or ".." in Path(config.command[2]).parts:
            raise SupervisorV7Error("Supervisor controller command must use the sealed relative path")
        ...
        if sha256_file(resolved_controller) != controller_binding["sha256"]:
            raise SupervisorV7Error("Supervisor controller binding drifted before child creation")
    output_paths = (config.launch_receipt, config.exit_receipt, config.failure_receipt)
    if len(set(output_paths)) != len(output_paths):
        raise SupervisorV7Error("Supervisor output paths overlap")
    for output_path in output_paths:
        candidate = _safe_output_path(root, output_path)
        if candidate.exists() or candidate.is_symlink():
            raise SupervisorV7Error("Supervisor output already exists or is unsafe")
    return root, bindings


def _launch_payload(
    config: SupervisorConfigV7,
    *,
    launch_id: str,
    ...
    if any(assignment_audit.get(name) != expected for name, expected in required.items()):
        raise SupervisorV7Error("Pre-resume Job assignment audit drifted")
    if len(launch_id) != 64 or any(character not in "0123456789abcdef" for character in launch_id):
        raise SupervisorV7Error("Supervisor launch id is invalid")
    ...
    if (
        ...
        raise SupervisorV7Error("Pre-resume Job assignment identity is invalid")
    if (
        ...
        raise SupervisorV7Error("Supervisor cannot prove its process identity")
    return {
...
def _failure_payload(
    ...
    *,
    ...
    if stage not in {"pre_resume", "post_resume"}:
        raise SupervisorV7Error("Supervisor failure stage is invalid")
    code = error.win32_error_code if isinstance(error, WindowsJobV7Error) else None
    operation = error.operation if isinstance(error, WindowsJobV7Error) else type(error).__name__
    return {
...
def run_supervised_v7(
    config: SupervisorConfigV7,
    *,
    environment: Mapping[str, str] | None = None,
    job_factory: Callable[..., WindowsJobV7] = WindowsJobV7.create,
    supervisor_identity_provider: Callable[[], Mapping[str, Any]] = _current_process_identity,
) -> SupervisedRunResultV7:
    ...
    child_environment.update(
        {
            "PYTHONUNBUFFERED": "1",
            "AXON_LOOP167_V7_LAUNCH_ID": launch_id,
            "AXON_LOOP167_V7_LAUNCH_RECEIPT": str(config.launch_receipt),
            "AXON_LOOP167_V7_SUPERVISOR_PID": str(supervisor_identity.get("pid", "")),
        }
    )
    job: WindowsJobV7 | None = None
    child: Any | None = None
    ...
    try:
        ...
        if active_after != 0:
            ...
            raise SupervisorV7Error("Controller exited while contained descendants remained")
        exit_payload = {
        ...
        if returncode != 0:
            raise SupervisorV7Error(f"Contained controller exited nonzero: {returncode}")
        return SupervisedRunResultV7(
        ...
        try:
            ...
            pass
        if isinstance(error, SupervisorV7Error):
            raise
        raise SupervisorV7Error("Supervisor failed before a contained controller completion") from error
    finally:
...
def validate_launch_receipt_v7(
    root: Path,
    path: Path,
    *,
    mode: str,
...
) -> ValidatedLaunchReceiptV7:
    """Validate the pre-resume receipt before a child can reach a v7 lease."""
    ...
    try:
        ...
        raise SupervisorV7Error("Pre-resume launch receipt is unavailable") from error
    if not isinstance(payload, dict):
        raise SupervisorV7Error("Pre-resume launch receipt is not an object")
    if (
        ...
        raise SupervisorV7Error("Pre-resume launch receipt identity drifted")
    if canonical_json_bytes(payload) != raw:
        raise SupervisorV7Error("Pre-resume launch receipt is not canonical")
    ...
    if not isinstance(launch_id, str) or len(launch_id) != 64 or any(
        ...
        raise SupervisorV7Error("Pre-resume launch receipt launch id is invalid")
    if expected_launch_id is not None and launch_id != expected_launch_id:
        raise SupervisorV7Error("Pre-resume launch receipt launch id drifted")
    command = payload.get("command")
    if not isinstance(command, list) or any(not isinstance(item, str) for item in command):
        raise SupervisorV7Error("Pre-resume launch receipt command is invalid")
    if payload.get("command_sha256") != _command_sha256(command):
        raise SupervisorV7Error("Pre-resume launch receipt command hash drifted")
    ...
    if (
        ...
        raise SupervisorV7Error("Pre-resume launch receipt supervisor identity drifted")
    if expected_supervisor_pid is not None and supervisor_identity["pid"] != expected_supervisor_pid:
        raise SupervisorV7Error("Pre-resume launch receipt supervisor PID drifted")
    ...
    if not isinstance(bindings, dict) or bindings != {
        ...
        raise SupervisorV7Error("Pre-resume launch receipt static bindings drifted")
    audit = payload.get("pre_resume_assignment")
    if not isinstance(audit, dict) or audit.get("assigned_before_resume") is not True:
        raise SupervisorV7Error("Pre-resume launch receipt lacks assignment proof")
    expected_assignment = {
        "creation_mode": "create_process_suspended_assign_verify_resume",
        "assignment_api": "AssignProcessToJobObject",
        "membership_api": "IsProcessInJob",
    ...
    if any(audit.get(name) != value for name, value in expected_assignment.items()):
        raise SupervisorV7Error("Pre-resume launch receipt assignment state drifted")
    ...
    if (
        ...
        raise SupervisorV7Error("Pre-resume launch receipt child identity is invalid")
    if expected_pid is not None and receipt_pid != expected_pid:
        raise SupervisorV7Error("Pre-resume launch receipt child PID drifted")
    if expected_creation_time_filetime is not None and receipt_creation != expected_creation_time_filetime:
        raise SupervisorV7Error("Pre-resume launch receipt child creation time drifted")
    canonical_sha256 = hashlib.sha256(raw).hexdigest()
    if sha256_file(receipt_path) != canonical_sha256:
        raise SupervisorV7Error("Pre-resume launch receipt changed during validation")
    return ValidatedLaunchReceiptV7(
