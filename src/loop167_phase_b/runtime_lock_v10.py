"""Pinned runtime facts for a contained Loop167 Phase-B v10 controller."""
...
def _clean_external_import_state() -> None:
    ...
    if loaded:
        raise PhaseBContractError("v10 runtime lock checked after external imports: " + ", ".join(loaded))
...
def _python_binding(root: Path) -> dict[str, str]:
    ...
    if relative != "vnev/Scripts/python.exe":
        raise PhaseBContractError("v10 runtime Python is outside vnev")
    return {
...
def _module_binding(root: Path, distribution: str, module_name: str) -> dict[str, str]:
    ...
    if not isinstance(module_file, str):
        raise PhaseBContractError(f"v10 runtime module lacks a file: {module_name}")
    ...
    if not relative.startswith("vnev/"):
        raise PhaseBContractError(f"v10 runtime module is outside vnev: {module_name}")
    return {
...
def _source_binding(root: Path, binding: Mapping[str, str], *, expected_path: str, label: str) -> dict[str, str]:
    ...
    if relative != expected_path:
        raise PhaseBContractError(f"v10 runtime lock {label} path drifted")
    return {"path": relative, "sha256": digest}


def build_runtime_lock_payload_v10(
    root: Path | str,
    *,
    ...
    validate_runtime_envelope_v10(root_path)
    ...
    return {
        ...
        "canonical_argv": {
            f"{role}_{mode}": list(canonical_argv_v10(role, mode))
            for role in ("supervisor", "controller")
            for mode in ("preflight", "execute")
        },
        "canonical_argv_sha256": canonical_argv_hashes_v10(),
        "thread_environment": dict(THREAD_ENVIRONMENT_V10),
...
def validate_runtime_lock_v10(
    ...
    *,
    ...
    if set(payload) != expected_keys or payload.get("schema") != RUNTIME_LOCK_SCHEMA:
        raise PhaseBContractError("v10 runtime lock schema drifted")
    ...
    if payload.get("thread_environment") != THREAD_ENVIRONMENT_V10:
        raise PhaseBContractError("v10 runtime lock thread environment drifted")
    if payload.get("isolated_python_required") is not True:
        raise PhaseBContractError("v10 runtime lock must require isolated Python")
    if payload.get("network_fetch_allowed") is not False or payload.get("dependency_install_allowed") is not False:
        raise PhaseBContractError("v10 runtime lock permits unsafe runtime actions")
    if verify_current_runtime:
        _clean_external_import_state()
        validate_current_runtime_invocation_v10(root_path, role=role, mode=mode)
    else:
        validate_thread_environment_v10()
    ...
    if payload.get("controller") != expected_controller or payload.get("supervisor") != expected_supervisor:
        raise PhaseBContractError("v10 runtime lock source binding drifted")
    ...
    expected_argv = {
        f"{role_name}_{mode_name}": list(canonical_argv_v10(role_name, mode_name))
        for role_name in ("supervisor", "controller")
        for mode_name in ("preflight", "execute")
    }
    if payload.get("canonical_argv") != expected_argv or payload.get("canonical_argv_sha256") != canonical_argv_hashes_v10():
        raise PhaseBContractError("v10 runtime lock argv drifted")
    if payload.get("python") != _python_binding(root_path):
        raise PhaseBContractError("v10 runtime lock Python binding drifted")
    ...
    if payload.get("packages") != expected_packages:
        raise PhaseBContractError("v10 runtime lock package binding drifted")
    return VerifiedRuntimeLockV10(
        role=role,
        mode=mode,
