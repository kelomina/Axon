"""Typed Windows Job Object boundary for Loop167 Phase-B v12.
...
class WindowsJobV12Error(RuntimeError):
...
class _Kernel32JobApiV12:
    """Small, fully typed subset of kernel32 used by the v12 Job boundary."""

    def __init__(self) -> None:
        if os.name != "nt":
            raise WindowsJobV12Error("Windows Job Objects are unavailable")
    ...
    @staticmethod
    def _failure(operation: str) -> WindowsJobV12Error:
        return WindowsJobV12Error(operation, int(ctypes.get_last_error()))
    ...
    def _environment_block(environment: Mapping[str, str]) -> Any:
        ...
        for key in sorted(environment, key=str.casefold):
            ...
            if (
                ...
                raise WindowsJobV12Error("CreateProcessW environment is invalid")
    ...
    def create_suspended_process(
        ...
        *,
        ...
        if not command or any(not isinstance(item, str) or not item for item in command):
            raise WindowsJobV12Error("CreateProcessW command is invalid")
        try:
            executable = Path(command[0]).resolve(strict=True)
            working_directory = Path(cwd).resolve(strict=True)
        except OSError as error:
            raise WindowsJobV12Error("CreateProcessW executable or cwd is unavailable") from error
        ...
        if not _handle_is_valid(process_information.hProcess) or not _handle_is_valid(
            ...
            raise WindowsJobV12Error("CreateProcessW returned invalid handles")
        return SuspendedProcessV12(
            api=self,
    ...
    def wait_process(self, process_handle: Any, timeout_seconds: float) -> int:
        if not math.isfinite(timeout_seconds) or timeout_seconds <= 0:
            raise WindowsJobV12Error("WaitForSingleObject timeout is invalid")
        ...
        if int(exit_code.value) == STILL_ACTIVE:
            raise WindowsJobV12Error("GetExitCodeProcess reported an active process after wait")
...
class SuspendedProcessV12:
    ...
    def resume(self) -> None:
        if self.resumed:
            raise WindowsJobV12Error("Suspended process was already resumed")
    ...
    def wait(self, timeout_seconds: float) -> int:
        if not self.resumed:
            raise WindowsJobV12Error("Cannot wait for an unresumed child")
...
class WindowsJobV12:
    ...
    def create(
        ...
        *,
        ...
        job_api = _Kernel32JobApiV12() if api is None else api
        ...
        try:
            ...
            if job_api.limit_flags(handle) != expected_flags:
                raise WindowsJobV12Error("QueryInformationJobObject returned unexpected limit flags")
            return job
        except Exception:
            try:
                job.close()
    ...
    def _require_open(self) -> None:
        if self._closed:
            raise WindowsJobV12Error("Windows Job Object is closed")

    def assign_current_process(self) -> dict[str, int | bool | str]:
        """Assign and verify the exact active process before the v12 lease boundary."""
        ...
        if not self._api.is_process_in_job(process_handle, self.handle):
            raise WindowsJobV12Error("IsProcessInJob returned false after assignment")
        return {
        ...
        *,
        ...
        try:
            ...
            if not self._api.is_process_in_job(process.process_handle, self.handle):
                raise WindowsJobV12Error("IsProcessInJob returned false after child assignment")
            return process
        except Exception:
            try:
                process.terminate()
    ...
    def assignment_audit(self, process: SuspendedProcessV12) -> dict[str, int | bool | str]:
        """Return only evidence that was true before the child was resumed."""

        self._require_open()
        if process.resumed:
            raise WindowsJobV12Error("Child was resumed before pre-resume audit")
        return {
    ...
    def active_processes(self) -> int:
        ...
        if isinstance(active, bool) or not isinstance(active, int) or active < 0:
            raise WindowsJobV12Error("QueryInformationJobObject returned invalid active count")
        return active
    ...
    def terminate(self, *, exit_code: int = 1) -> int:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise WindowsJobV12Error("TerminateJobObject left active child processes")
            time.sleep(0.05)
    ...
    def __enter__(self) -> "WindowsJobV12":
        self._require_open()
        return self
...
class WindowsJobAssignmentProbeV12:
...
def probe_windows_job_assignment_v12(
    *,
    memory_limit_bytes: int,
    job_factory: Callable[..., WindowsJobV12] = WindowsJobV12.create,
) -> WindowsJobAssignmentProbeV12:
    ...
    except WindowsJobV12Error as error:
        return WindowsJobAssignmentProbeV12(
            ready=False,
...
__all__ = [
    ...
    "WindowsJobV12",
    "WindowsJobV12Error",
    "probe_windows_job_assignment_v12",
]
