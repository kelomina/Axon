"""Raw-free static validation for the Loop167 Phase-B v12 control plane."""
...
SOURCE_CLOSURE_SCHEMA = "axon_loop167_phase_b_source_closure_v12"
V12_SCOPE = "v8_in_process_fresh_authorization_remediation_single_authorized_train_only_execution_no_heldout_access"
...
EXPECTED_BLOCKERS = [
    "fresh_resource_guard_v8_not_sealed",
    "run_authorization_v8_not_sealed",
    "contained_child_pre_resume_receipt_not_created",
    "child_job_attestation_not_created",
    "one_shot_lease_v8_not_consumed",
]
REQUIRED_SOURCE_PATHS = frozenset(
    {
        ...
        "src/loop167_phase_b/execution_contract_v12.py",
        "src/loop167_phase_b/windows_job_v12.py",
        "src/loop167_phase_b/supervisor_v12.py",
        "src/loop167_phase_b/invocation_v12.py",
        "src/loop167_phase_b/runtime_lock_v12.py",
        "src/loop167_phase_b/preflight_v12.py",
        "src/loop167_phase_b/resource_guard_v12.py",
        "src/loop167_phase_b/execution_authorization_v12.py",
        "src/loop167_phase_b/launch_authorization_v12.py",
        "src/loop167_phase_b/lease_v12.py",
        "src/loop167_phase_b/child_attestation_v12.py",
        "src/loop167_phase_b/loop166_v8_bridge.py",
        "src/loop166/windows_job.py",
        "src/loop166/windows_process_lineage.py",
        "scripts/run_loop167_phase_b_supervisor_v12.py",
        "scripts/run_loop167_phase_b_controller_v12.py",
    }
)


@dataclass(frozen=True)
class StaticPreflightV8Receipt:
...
def _verify_source_files(root: Path, source_files: object) -> None:
    if not isinstance(source_files, list) or not source_files:
        raise PhaseBContractError("v12 source closure has no source-file bindings")
    observed: set[str] = set()
    for binding in source_files:
        path, _ = verify_safe_file_binding(root, binding, label="v8_source_file")
        relative = safe_project_relative_path(root, path, require_exists=True, require_regular_file=True)
        if relative in observed:
            raise PhaseBContractError("v12 source closure repeats a source file")
        observed.add(relative)
    if not REQUIRED_SOURCE_PATHS.issubset(observed):
        raise PhaseBContractError("v12 source closure omits required control-plane sources")
...
def _validate_source_closure(
    ...
    *,
    ...
    if set(payload) != expected_keys or payload.get("schema") != SOURCE_CLOSURE_SCHEMA:
        raise PhaseBContractError("v12 source closure schema drifted")
    if payload.get("loop_id") != LOOP_ID or payload.get("scope") != V12_SCOPE:
        raise PhaseBContractError("v12 source closure identity drifted")
    if payload.get("static_preflight_ready") is not True or payload.get("phase_b_raw_execution_ready") is not False:
        raise PhaseBContractError("v12 source closure readiness drifted")
    if payload.get("dynamic_execution_gates") != EXPECTED_DYNAMIC_GATES:
        raise PhaseBContractError("v12 source closure dynamic gates drifted")
    if payload.get("remaining_execution_blockers") != EXPECTED_BLOCKERS:
        raise PhaseBContractError("v12 source closure blockers drifted")
    _binding(payload.get("parent_v10_prelease_attestation"), label="parent_attestation", expected_path=PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH)
    ...
    if _binding(payload.get("controller"), label="controller", expected_path=CONTROLLER_RELATIVE_PATH) != dict(controller_binding):
        raise PhaseBContractError("v12 source closure controller binding drifted")
    if _binding(payload.get("supervisor"), label="supervisor", expected_path=SUPERVISOR_RELATIVE_PATH) != dict(supervisor_binding):
        raise PhaseBContractError("v12 source closure supervisor binding drifted")
...
def _source_file_binding(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise PhaseBContractError(f"v12 source closure lacks a unique {label} binding")
    _path, digest = verify_safe_file_binding(root, candidates[0], label=label)
    return {"path": expected_path, "sha256": digest}


def validate_static_preflight_v12(
    root: Path | str,
    *,
    source_closure_binding: Mapping[str, str],
    controller_binding: Mapping[str, str],
    ...
    if phase not in {"prelaunch", "contained_child", "attested_child", "leased_child_pre_raw"}:
        raise PhaseBContractError("v12 static preflight phase is invalid")
    ...
    if sha256_file(_controller_path) != normalized_controller["sha256"]:
        raise PhaseBContractError("v12 controller changed during static preflight")
    if sha256_file(_supervisor_path) != normalized_supervisor["sha256"]:
        raise PhaseBContractError("v12 supervisor changed during static preflight")
    ...
    verify_parent_v10_prelease_attestation_v12(root_path, closure["parent_v10_prelease_attestation"])
    contract = verify_execution_contract_v12(root_path, closure["phase_b_execution_contract"])
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v12" or runtime_lock.get("loop_id") != LOOP_ID:
    ...
    if runtime_lock.get("controller") != normalized_controller or runtime_lock.get("supervisor") != normalized_supervisor:
        raise PhaseBContractError("v12 runtime lock controller bindings drifted")
    ...
    if phase == "prelaunch":
        assert_output_catalog_is_fresh_v12(root_path, contract.output_catalog)
    elif phase == "contained_child":
        assert_contained_child_prelease_surface_v12(root_path, contract.output_catalog)
    elif phase == "attested_child":
        assert_attested_child_prelease_surface_v12(root_path, contract.output_catalog)
    else:
        assert_leased_child_pre_raw_surface_v12(root_path, contract.output_catalog)
    return StaticPreflightV8Receipt(
