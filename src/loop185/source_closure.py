"""Loop185 源码闭包验证器（与 Loop184 一致，仅修改扫描路径）。"""
...
_LOOP185_DIR: Final[Path] = _PROJECT_ROOT / "src" / "loop185"
...
class ClosureReport:
    ...
    def assert_passed(self) -> None:
        if not self.passed:
            lines = ["Loop185 source closure failed:"]
...
def scan_source_closure(
    *,
    ...
    for path in _LOOP185_DIR.rglob("*.py"):
