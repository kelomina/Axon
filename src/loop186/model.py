"""Independent Multi-Scale HGConv-Region model for Loop186.

Loop186 架构（Loop185 扩容 2x）：
- byte_embedding → patchify (Conv1d) → 4 × MultiScaleHGConvBlocks → attention pooling
- → 8 层 region_transformer → fusion_head

参数量：~10M（Loop185: 2.61M → 4x 扩容）
"""
...
class HGConvRegionNet(nn.Module):
    """Loop186 Multi-Scale HGConv-Region 模型（Loop185 扩容 2x）。"""
    ...
    def _validate_inputs(
        ...
        if b0_features is not None:
            ...
            if not torch.is_floating_point(b0_features):
                raise ValueError("b0_features must be finite floating values")
            if cfg.runtime_checks and not torch.isfinite(b0_features).all().item():
                raise ValueError("b0_features must be finite floating values")
    ...
    def _encode_patches(self, tokens: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.use_checkpointing and self.training:
            ...
            def _checkpointed_forward(p: torch.Tensor, pm: torch.Tensor) -> torch.Tensor:
                for blk in self.blocks:
                    p = blk(p, pm)
                return p

            patches = torch.utils.checkpoint.checkpoint(
                _checkpointed_forward, patches, patch_mask,
                use_reentrant=False,
            )
        else:
    ...
    def forward(
        ...
        if self.config.runtime_checks and any(
            not torch.isfinite(values).all().item() for values in output.values()
        ):
