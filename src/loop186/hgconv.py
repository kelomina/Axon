"""PyTorch Multi-Scale HGConv core for the Loop185 lineage.

架构与 Loop184 一致：多尺度并行 HGConvBlock（filter_lengths=[8, 16, 32, 64]）。
Loop185 改进聚焦于 4-fold OOF 集成 + 关闭 SAM 加速，不在 HGConv block 层面改动。
"""
...
def _require_real_floating(
    ...
    *,
    ...
    if runtime_checks and not torch.isfinite(values).all().item():
...
def _fft_circular_convolution(
    ...
    *,
    ...
    if runtime_checks and not torch.isfinite(output).all().item():
...
def circular_convolution(
    ...
    *,
    ...
    _require_real_floating(left, name="left", runtime_checks=runtime_checks)
    _require_real_floating(right, name="right", runtime_checks=runtime_checks)
    normalized_dim = _normalize_dim(left, dim)
    right_dim = normalized_dim - (left.ndim - right.ndim)
    if not 0 <= right_dim < right.ndim:
        raise ValueError("right tensor does not expose the convolution axis")
    ...
    return _fft_circular_convolution(
        left,
        right,
        dim=normalized_dim,
...
def malware_kernel_precondition(
    ...
    *,
    ...
    if runtime_checks and not torch.isfinite(kernel).all().item():
...
class HGConvConfig:
    """单尺度 HGConv 配置（保留用于回退对比，Loop185 不使用）。"""
...
class HGConvBlock(nn.Module):
    """单尺度 HGConvBlock（保留用于回退对比，Loop185 不使用）。"""
    ...
    def forward(self, patches: torch.Tensor, patch_mask: torch.Tensor) -> torch.Tensor:
        ...
        convolved = _fft_circular_convolution(
            bound,
            kernel,
            dim=1,
            runtime_checks=self.config.runtime_checks,
        ...
        if self.config.runtime_checks and not torch.isfinite(output).all().item():
...
class MultiScaleHGConvBlock(nn.Module):
    """多尺度并行 HGConvBlock（与 Loop184 架构一致）。

    对每个 filter_length 独立执行：bind -> convolve(filter_i) -> bias+gelu -> unbind -> norm
    ...
    def _fast_forward(
        ...
        dim = self.config.model_dim
        max_filter_length = max(self.config.filter_lengths)
        kernels = []
        for conv_filter, filter_length in zip(
            self.convolution_filters,
            self.config.filter_lengths,
        ):
            kernel = conv_filter.squeeze(0).transpose(0, 1)
            left = (max_filter_length - filter_length) // 2
            right = max_filter_length - filter_length - left
            kernels.append(functional.pad(kernel, (left, right)))
        grouped_kernel = (
            torch.stack(kernels, dim=1)
            .reshape(dim * self.num_scales, 1, max_filter_length)
        )
        bound = values * self.bind_filter.view(1, 1, dim)
        bound_t = bound.transpose(1, 2)
        left = (max_filter_length - 1) // 2
        right = max_filter_length - 1 - left
        convolved = functional.conv1d(
            functional.pad(bound_t, (left, right)),
            grouped_kernel,
            groups=dim,
        )
        convolved = (
            convolved.view(values.shape[0], dim, self.num_scales, values.shape[1])
            .permute(0, 2, 3, 1)
        )
        biases = torch.stack(
            [bias.squeeze(0).squeeze(0) for bias in self.bias_weights],
            dim=0,
        ).view(1, self.num_scales, 1, dim)
        convolved = functional.gelu(convolved + bound.unsqueeze(1) * biases)
        scale_probs = functional.softmax(self.scale_weights, dim=0)
        fused = torch.sum(convolved * scale_probs.view(1, -1, 1, 1), dim=1)
        unbound = fused * self.unbind_filter.view(1, 1, dim)
        gated = functional.glu(self.glu_projection(self.norm(unbound)), dim=-1)
        return (values.to(dtype=gated.dtype) + self.dropout(gated)) * mask
    ...
    def forward(self, patches: torch.Tensor, patch_mask: torch.Tensor) -> torch.Tensor:
        ...
        residual = values
        if self.config.fast:
            output = self._fast_forward(values, mask)
            if self.config.runtime_checks and not torch.isfinite(output).all().item():
        ...
        for conv_filter, bias_weight in zip(
            self.convolution_filters, self.bias_weights
        ):
            kernel = malware_kernel_precondition(
                conv_filter,
