"""Loop186 源码闭包验证器（与 Loop185 一致，仅修改扫描路径）。"""
...
_LOOP186_DIR: Final[Path] = _PROJECT_ROOT / "src" / "loop186"
...
class ClosureReport:
    ...
    def assert_passed(self) -> None:
        if not self.passed:
            lines = ["Loop186 source closure failed:"]
...
def scan_source_closure(
    *,
    ...
    for path in _LOOP186_DIR.rglob("*.py"):
