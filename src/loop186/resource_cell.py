"""Loop185 资源门框架（与 Loop184 一致，仅修改 loop_id）。"""
...
def deadline_check_due(completed_steps: int) -> bool:
    return completed_steps > 0


def enforce_epoch_deadline(
    *,
    ...
    if elapsed_seconds > hard_seconds:
        raise TimeoutError(
            f"epoch wall {elapsed_seconds:.1f}s exceeds {hard_seconds:.1f}s"
        )
    if completed_steps <= 0 or completed_steps > total_steps:
        raise ValueError("completed_steps must be in [1, total_steps]")
    projected_seconds = elapsed_seconds * total_steps / completed_steps
    if projected_seconds > projection_seconds:
        raise TimeoutError(
            f"projected epoch wall {projected_seconds:.1f}s exceeds "
            f"{projection_seconds:.1f}s"
        )
...
class ResourceCell:
    ...
    def inject_sample(self, sample: ResourceSample) -> None:
        ...
        self._check_sample(sample, previous=previous)

    def _check_sample(
        self,
        sample: ResourceSample,
        *,
        previous: ResourceSample | None,
    ) -> None:
        ...
        if sample.epoch > 0:
            previous_wall = (
                ...
                if previous is not None and previous.epoch == sample.epoch - 1
                else 0.0
            )
            epoch_wall_seconds = sample.wall_seconds - previous_wall
            if epoch_wall_seconds > self.budget.epoch_wall_seconds:
                self.violations.append(ResourceViolation(
                    kind="epoch_wall_over",
                    sample=sample,
                    threshold=float(self.budget.epoch_wall_seconds),
                    actual=float(epoch_wall_seconds),
                    detail=(
                        f"epoch wall {epoch_wall_seconds} > budget "
                        f"{self.budget.epoch_wall_seconds}"
                    ),
                ))
    ...
    def build_receipt(self) -> dict[str, object]:
        return {
            ...
            "budget": {
                ...
                "epoch_wall_seconds": self.budget.epoch_wall_seconds,
...
def assert_budget_invariants() -> None:
    ...
    assert gate.epoch_wall_seconds > 0, "epoch wall budget must be positive"
    assert gate.wall_seconds >= gate.max_epochs * gate.epoch_wall_seconds, (
        "wall budget must cover every epoch budget"
    )
