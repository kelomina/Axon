#!/usr/bin/env python3
"""Seal the complete v6 source closure for the Loop167 Phase-B execution route."""
...
from loop167_phase_b.preflight_v6 import (  # noqa: E402
    EXPECTED_BLOCKERS,
...
V6_SOURCE_PATHS = (
    ...
    "scripts/build_loop167_phase_b_execution_contract_v6.py",
    "scripts/build_loop167_phase_b_runtime_lock_v6.py",
    "scripts/build_loop167_phase_b_resource_guard_v6.py",
    "scripts/build_loop167_phase_b_execution_authorization_v6.py",
    "scripts/run_loop167_phase_b_supervisor_v6.py",
    "scripts/run_loop167_phase_b_controller_v6.py",
    "scripts/probe_loop167_phase_b_v6_suspended_child.py",
    "scripts/seal_loop167_phase_b_source_closure_v6.py",
    "tests/test_loop167_phase_b_package_import.py",
    "tests/test_loop167_phase_b_v6_windows_job.py",
    "tests/test_loop167_phase_b_v6_supervisor.py",
    "tests/test_loop167_phase_b_v6_child_attestation.py",
    "tests/test_loop167_phase_b_v6_loop166_bridge.py",
    "tests/test_loop167_phase_b_v6_path_safety.py",
    "tests/test_loop167_phase_b_v6_resource_guard.py",
    "tests/test_loop167_phase_b_v6_execution_authorization.py",
    "tests/test_loop167_phase_b_lease_v6.py",
    "tests/test_loop167_phase_b_v6_controller.py",
    "tests/test_loop167_phase_b_v6_source_closure.py",
    "tests/test_loop167_phase_b_v6_state_machine.py",
    "tests/test_loop167_phase_b_v6_windows_integration.py",
...
def _validate_static_v6_artifacts() -> None:
    parent_attestation = _binding(PARENT_V5_PRELEASE_ATTESTATION_RELATIVE_PATH)
    verify_parent_v5_prelease_attestation_v6(PROJECT_ROOT, parent_attestation)
    contract_binding = _binding(EXECUTION_CONTRACT_RELATIVE_PATH)
    verify_execution_contract_v6(PROJECT_ROOT, contract_binding)
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v6" or runtime_lock.get("loop_id") != LOOP_ID:
        raise ValueError("Phase-B runtime lock v6 schema drifted")
    if runtime_lock.get("controller") != _binding(CONTROLLER_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v6 controller binding drifted")
    if runtime_lock.get("supervisor") != _binding(SUPERVISOR_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v6 supervisor binding drifted")
    if runtime_lock.get("execution_contract") != contract_binding:
        raise ValueError("Phase-B runtime lock v6 execution-contract binding drifted")


def build_source_closure_v6_payload() -> dict[str, Any]:
    _validate_static_v6_artifacts()
    if len(V6_SOURCE_PATHS) != len(set(V6_SOURCE_PATHS)):
        raise ValueError("v6 source closure repeats a path")
    if not REQUIRED_SOURCE_PATHS.issubset(set(V6_SOURCE_PATHS)):
        raise ValueError("v6 source closure omits a required control-plane path")
    return {
        "schema": SOURCE_CLOSURE_SCHEMA,
        "loop_id": LOOP_ID,
        "scope": V6_SCOPE,
        "parent_v5_prelease_attestation": _binding(PARENT_V5_PRELEASE_ATTESTATION_RELATIVE_PATH),
        ...
        "source_files": [_binding(relative_path) for relative_path in V6_SOURCE_PATHS],
