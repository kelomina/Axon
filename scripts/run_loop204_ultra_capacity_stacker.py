"""Loop204: Deep Ultra-High Capacity Stacker Evaluation Script.

Evaluates ultra-capacity stacker across Val (20k), Test-10k (10k), and Full-test (160k).
Generates reports/roadmap_9997/loop204_ultra_capacity_receipt.json.
"""
...
from src.loop204_ultra_capacity_stacker import Loop204UltraCapacityStacker
...
def evaluate_loop204_split(split: str, stacker: Loop204UltraCapacityStacker):
    ...
    for sha, row in data.items():
        ...
        stacker_pred, _ = stacker(primary_prob, l202_prob, l197_prob, l203_entropy, auth_status, signer_subject)
...
def main():
    print("=" * 70)
    print("Axon v2.6 - Loop204 Deep Ultra-High Capacity Stacker Evaluation")
    print("=" * 70)

    stacker = Loop204UltraCapacityStacker()
    report = {}

    for split in ("val", "test10k", "full"):
        res = evaluate_loop204_split(split, stacker)
        if res:
            report[split] = res
            print(f"[{split}] Baseline Errors: {res['loop151_baseline']['total_errors']} -> Ultra Stacker Errors: {res['loop204_ultra_stacker']['total_errors']} | Repairs: {res['transitions']['repairs']} | Breaks: {res['transitions']['breaks']} | Net Repairs: +{res['transitions']['net_repairs']} | Stacker F1: {res['loop204_ultra_stacker']['f1']:.6f}")

    report_path = Path(__file__).resolve().parent.parent / "reports" / "roadmap_9997" / "loop204_ultra_capacity_receipt.json"
