"""Loop213: Sigmoidal Annealing Probability Calibration Evaluation Script.

Evaluates sigmoidal temperature annealing calibration.
Generates reports/roadmap_9997/loop213_annealed_calibrator_receipt.json.
"""
...
def evaluate_calibrator_split(split: str, calibrator: Loop213AnnealedCalibrator):
    ...
    for sha, row in data.items():
        ...
        if base_pred != label and calib_pred == label:
            repairs += 1
        elif base_pred == label and calib_pred != label:
            breaks += 1
...
def main():
    print("=" * 70)
    print("Axon v2.6 - Loop213 Sigmoidal Annealing Calibration Evaluation")
    print("=" * 70)
    ...
    for split in ("val", "test10k", "full"):
        res = evaluate_calibrator_split(split, calibrator)
        if res:
            report[split] = res
            print(f"[{split}] Base Errors: {res['base_metrics']['total_errors']} -> Calibrated Errors: {res['calibrated_metrics']['total_errors']} | Repairs: {res['transitions']['repairs']} | Breaks: {res['transitions']['breaks']} | Net: {res['transitions']['net_repairs']}")

    report_path = Path(__file__).resolve().parent.parent / "reports" / "roadmap_9997" / "loop213_annealed_calibrator_receipt.json"
