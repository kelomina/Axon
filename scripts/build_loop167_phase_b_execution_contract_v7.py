#!/usr/bin/env python3
"""Build or verify the immutable Loop167 Phase-B v7 execution contract."""
...
def build_payload(*, parent_v6_prelease_attestation_binding: dict[str, str]) -> dict[str, Any]:
    return build_execution_contract_payload_v7(
        safe_project_root(PROJECT_ROOT),
        parent_v6_prelease_attestation_binding=parent_v6_prelease_attestation_binding,
    )
...
def main() -> int:
    ...
    root = safe_project_root(PROJECT_ROOT)
    attestation_path = safe_project_path(root, PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH, require_exists=False)
    attestation_payload = build_parent_v6_prelease_attestation_payload_v7(root)
    expected_attestation = canonical_json_bytes(attestation_payload)
    if args.write_parent_attestation:
        output_path = ensure_v7_static_artifact_parent(root, PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH)
        digest = _write_new(output_path, attestation_payload)
        print(json.dumps({"path": PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH, "sha256": digest}, sort_keys=True))
        return 0
    ...
    payload = build_payload(parent_v6_prelease_attestation_binding=_binding(PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH))
    expected_contract = canonical_json_bytes(payload)
    if args.write_contract:
        output_path = ensure_v7_static_artifact_parent(root, EXECUTION_CONTRACT_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not contract_path.is_file() or contract_path.read_bytes() != expected_contract:
            raise SystemExit("Phase-B execution contract v7 is missing or drifted")
