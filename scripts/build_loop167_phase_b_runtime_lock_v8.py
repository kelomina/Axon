#!/usr/bin/env python3
"""Build or verify the isolated runtime lock for the Loop167 Phase-B v8 route."""
...
from loop167_phase_b.invocation_v8 import bootstrap_thread_environment_v8  # noqa: E402
from loop167_phase_b.path_safety_v4 import safe_project_path, safe_project_root  # noqa: E402
from loop167_phase_b.runtime_lock_v8 import (  # noqa: E402
...
def build_payload() -> dict[str, Any]:
    ...
    return build_runtime_lock_payload_v8(
...
def main() -> int:
    ...
    if args.write:
        output_path = ensure_v8_static_artifact_parent(root, RUNTIME_LOCK_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not output_path.is_file() or output_path.read_bytes() != canonical_json_bytes(payload):
            raise SystemExit("Phase-B runtime lock v8 is missing or drifted")
        validate_runtime_lock_v8(
            root,
