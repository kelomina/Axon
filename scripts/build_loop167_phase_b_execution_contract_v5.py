#!/usr/bin/env python3
"""Build or verify the immutable Loop167 Phase-B v5 execution contract."""
...
def build_payload(*, parent_v4_prelease_attestation_binding: dict[str, str]) -> dict[str, Any]:
    return build_execution_contract_payload_v5(
        safe_project_root(PROJECT_ROOT),
        protocol_binding=_binding(PHASE_B_PROTOCOL_RELATIVE_PATH),
        parent_v4_prelease_attestation_binding=parent_v4_prelease_attestation_binding,
    )
...
def main() -> int:
    ...
    root = safe_project_root(PROJECT_ROOT)
    attestation_path = safe_project_path(
        root,
        PARENT_V4_PRELEASE_ATTESTATION_RELATIVE_PATH,
        require_exists=False,
    )
    attestation_payload = build_parent_v4_prelease_attestation_payload_v5(root)
    expected_attestation = canonical_json_bytes(attestation_payload)
    if args.write_parent_attestation:
        attestation_path = ensure_v5_static_artifact_parent(root, PARENT_V4_PRELEASE_ATTESTATION_RELATIVE_PATH)
        digest = _write_new(attestation_path, attestation_payload)
        print(json.dumps({"path": PARENT_V4_PRELEASE_ATTESTATION_RELATIVE_PATH, "sha256": digest}, sort_keys=True))
        return 0
    if not attestation_path.is_file() or attestation_path.read_bytes() != expected_attestation:
        raise SystemExit("Parent v4 pre-lease attestation is missing or drifted")

    output_path = safe_project_path(root, EXECUTION_CONTRACT_RELATIVE_PATH, require_exists=False)
    payload = build_payload(
        parent_v4_prelease_attestation_binding=_binding(PARENT_V4_PRELEASE_ATTESTATION_RELATIVE_PATH),
    )
    expected = canonical_json_bytes(payload)
    if args.write_contract:
        output_path = ensure_v5_static_artifact_parent(root, EXECUTION_CONTRACT_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not output_path.is_file() or output_path.read_bytes() != expected:
            raise SystemExit("Phase-B execution contract v5 is missing or drifted")
