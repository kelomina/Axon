...
最高效的方案：
1. 从已提取的cache中随机抽取20000个样本（良性10k + 恶意10k）
2. 按1:1:8划分数据集
3. 直接运行6个实验（无需重新提取特征）
"""
...
def create_experiment_dataset_from_cache(
    cache_dir: Path,
    ...
    """从cache中创建实验数据集。"""
    print("\n" + "=" * 80)
    print("步骤1: 从cache中抽取样本")
    print("=" * 80)
    
    # 由于cache中的文件没有标签信息，我们需要从原始目录推断
    # 简单方案：从cache中随机抽取20000个，然后假设前10000个是良性，后10000个是恶意
    # 或者：读取manifest文件获取标签信息
    
    print(f"\n从 {cache_dir} 抽取样本...")
    all_samples = sample_from_cache(cache_dir, samples_per_class * 2, seed)
    
    # 简单分割（前一半良性，后一半恶意）
    # 注意：这只是近似，实际应该读取manifest获取真实标签
...
def run_experiment(
    ...
    try:
        ...
        args.fast = False  # 不使用fast_mode，让cache直接加载
...
def main():
    """主函数：运行所有对比实验。"""
    print("=" * 100)
    print("Axon v2.6 泛化增强对比实验 - 从cache抽样版")
    print("=" * 100)
    ...
    output_dir = PROJECT_ROOT / "models" / "comparison_experiments_from_cache"
    ...
    # 步骤1: 从cache中抽取样本
    benign_samples, malicious_samples = create_experiment_dataset_from_cache(
        cache_dir=cache_dir,
    ...
    # 步骤4: 运行实验（直接使用cache，无需特征提取）
    print(f"\n" + "=" * 100)
    print(f"步骤4: 运行实验")
    print("=" * 100)
    print(f"注意: 直接使用已提取的cache，无需重新提取特征")
