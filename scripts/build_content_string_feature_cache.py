#!/usr/bin/env python3
"""Precompute content-only string/keyword features for Stage-2 experiments."""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build sidecar cache for content-only string features.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if worker_count == 1:
        ...
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            for index, result in enumerate(executor.map(_build_one, payloads, chunksize=64), start=1):
                counts[result["status"]] += 1
                counts["zero_features"] += int(result["zero"])
                if index % 1000 == 0:
                    print(
                        f"[string-cache] processed={index}/{len(payloads)} created={counts['created']} "
    ...
    report = {
        "schema": "axon_content_string_feature_cache_v1",
        "protocol": "content-only binary string/keyword features; filename/path/extension are not encoded",
        "predictions": [str(resolve_path(path)) for path in args.predictions],
        "cache_dir": str(cache_dir),
        "workers": worker_count,
        "input_rows": len(rows),
        "unique_rows": len(unique_rows),
        "feature_dim": len(CONTENT_STRING_FEATURE_NAMES),
        "counts": counts,
        "elapsed_sec": elapsed,
    }
