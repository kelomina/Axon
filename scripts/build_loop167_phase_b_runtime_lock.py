#!/usr/bin/env python3
"""Build or verify the runtime lock for Loop167 Phase-B static preflight."""
...
from loop167_phase_b.runtime_lock import build_runtime_lock_payload  # noqa: E402
...
def build_payload() -> dict[str, Any]:
    if not CONTROLLER_PATH.is_file():
        raise FileNotFoundError("Phase-B controller must exist before runtime lock sealing")
...
def main() -> int:
    ...
    if args.write:
        ...
        if not RUNTIME_LOCK_PATH.is_file() or RUNTIME_LOCK_PATH.read_bytes() != expected:
            raise SystemExit("Phase-B runtime lock is missing or drifted")
