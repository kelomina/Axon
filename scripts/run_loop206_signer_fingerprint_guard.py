"""Loop206: Authenticode Certificate Serial & Fingerprint Guard Evaluation Script.

Evaluates certificate serial guard across Val (20k), Test-10k (10k), and Full-test (160k).
Generates reports/roadmap_9997/loop206_signer_fingerprint_receipt.json.
"""
...
def evaluate_fingerprint_split(split: str, guard: Loop206SignerFingerprintGuard):
    ...
    for sha, row in data.items():
        ...
        guard_pred, is_down = guard.evaluate_sample(base_pred, auth_status, cert_serial)
        if is_down:
            downgrades += 1
...
def main():
    print("=" * 70)
    print("Axon v2.6 - Loop206 Authenticode Signer Serial Guard Evaluation")
    print("=" * 70)
    ...
    for split in ("val", "test10k", "full"):
        res = evaluate_fingerprint_split(split, guard)
        if res:
            report[split] = res
    ...
    report_path = Path(__file__).resolve().parent.parent / "reports" / "roadmap_9997" / "loop206_signer_fingerprint_receipt.json"
