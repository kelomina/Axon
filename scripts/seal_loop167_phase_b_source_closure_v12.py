#!/usr/bin/env python3
"""Seal the complete v12 source closure for the Loop167 Phase-B execution route."""
...
from loop167_phase_b.preflight_v12 import (  # noqa: E402
    EXPECTED_BLOCKERS,
...
V12_SOURCE_PATHS = (
    ...
    "scripts/build_loop167_phase_b_execution_contract_v12.py",
    "scripts/build_loop167_phase_b_runtime_lock_v12.py",
    ...
    "scripts/seal_loop167_phase_b_source_closure_v12.py",
    ...
    "tests/test_loop167_phase_b_lease_v12.py",
...
def _validate_static_v8_artifacts() -> None:
    parent_attestation = _binding(PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH)
    verify_parent_v10_prelease_attestation_v12(PROJECT_ROOT, parent_attestation)
    contract_binding = _binding(EXECUTION_CONTRACT_RELATIVE_PATH)
    verify_execution_contract_v12(PROJECT_ROOT, contract_binding)
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v12" or runtime_lock.get("loop_id") != LOOP_ID:
        raise ValueError("Phase-B runtime lock v12 schema drifted")
    if runtime_lock.get("controller") != _binding(CONTROLLER_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v12 controller binding drifted")
    if runtime_lock.get("supervisor") != _binding(SUPERVISOR_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v12 supervisor binding drifted")
    if runtime_lock.get("execution_contract") != contract_binding:
        raise ValueError("Phase-B runtime lock v12 execution-contract binding drifted")


def build_source_closure_v8_payload() -> dict[str, Any]:
    _validate_static_v8_artifacts()
    if len(V12_SOURCE_PATHS) != len(set(V12_SOURCE_PATHS)):
        raise ValueError("v12 source closure repeats a path")
    if not REQUIRED_SOURCE_PATHS.issubset(set(V12_SOURCE_PATHS)):
        raise ValueError("v12 source closure omits a required control-plane path")
    return {
        "schema": SOURCE_CLOSURE_SCHEMA,
        "loop_id": LOOP_ID,
        "scope": V12_SCOPE,
        "parent_v10_prelease_attestation": _binding(PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH),
        ...
        "source_files": [_binding(relative_path) for relative_path in V12_SOURCE_PATHS],
...
def main() -> int:
    ...
    if args.write:
        output_path = ensure_v12_static_artifact_parent(root, SOURCE_CLOSURE_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not output_path.is_file() or output_path.read_bytes() != expected:
            raise SystemExit("Phase-B source closure v12 is missing or drifted")
