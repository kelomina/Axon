#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""强化版 Stage-2（良性扩充重训后）：完整特征 + HGB 集成。

与 stage2_739k_v2.py 同逻辑，仅改输入/输出目录到 benign。
...
输入：reports/full_739k_benign/{base_prob,content_pe_v1,content_v2string}/
输出：reports/full_739k_benign/stage2_report.json 追加 "stage2_v2" 段 + 控制台对比。
"""
...
CERT_DIR = PROJECT_ROOT / "reports" / "full_739k_benign" / "content_cert"
...
def main() -> None:
    t0 = time.time()
    print("=== Stage-2 v2 (benign): derived + content_v1 (+v2/string if ready) + HGB ensemble ===")
    ...
    print(f"[对比] 扩充前(331特征,旧模型) TEST F1=0.99341 errors=1511; 旧基座 TEST F1=0.9795 errors=4742")
    ...
    rep["stage2_v2"] = {
        ...
        "prev_expansion_stage2_f1": 0.99341, "prev_expansion_stage2_errors": 1511,
        "new_base_test_f1": 0.97648,
