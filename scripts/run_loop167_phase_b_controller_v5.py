#!/usr/bin/env python3
"""Run the only sealed Loop167 Phase-B v5 controller route."""
...
from loop167_phase_b.invocation_v5 import (  # noqa: E402
    bootstrap_thread_environment_v5,
    canonical_argv_v5,
    validate_current_runtime_invocation_v5,
)
from loop167_phase_b.preflight_v5 import validate_static_preflight_v5  # noqa: E402

SOURCE_CLOSURE_PATH = PROJECT_ROOT / SOURCE_CLOSURE_RELATIVE_PATH
RUN_AUTHORIZATION_PATH = PROJECT_ROOT / RUN_AUTHORIZATION_RELATIVE_PATH
EXECUTION_RECEIPT_PATH = PROJECT_ROOT / next(
    entry["path"] for entry in FIXED_OUTPUT_CATALOG if entry["name"] == "execution_receipt"
)
RECEIPT_SCHEMA = "axon_loop167_phase_b_execution_receipt_v5"
...
def _assert_authorization_matches_static_preflight(authorization: Any, static_receipt: Any) -> None:
    ...
    if tuple(authorization.canonical_execute_argv) != canonical_argv_v5("execute"):
...
def _execute_after_lease(
    *,
    ...
    return {
        ...
        "status": "completed_single_replacement_v5_train_only_raw_pass_fixed_oof_not_promotion_or_heldout_evaluation",
...
def _static_preflight(mode: str) -> Any:
    bootstrap_thread_environment_v5()
    validate_current_runtime_invocation_v5(PROJECT_ROOT, mode=mode)
    return validate_static_preflight_v5(
        PROJECT_ROOT,
...
def _verify_authorized_execution_contract(authorization: Any) -> Any:
    """Freeze the authorization binding before touching the raw-root, Job, or lease boundary."""

    return verify_execution_contract_v5(
        PROJECT_ROOT,
...
def run_execute() -> dict[str, Any]:
    ...
    from loop167_phase_b.execution_authorization_v5 import validate_execution_authorization_v5
    from loop167_phase_b.lease_v5 import (
        consume_execution_lease_v5,
        verify_consumed_execution_lease_v5,
    )
    ...
    contract = _verify_authorized_execution_contract(authorization)
    ...
    consumed_lease = consume_execution_lease_v5(
        PROJECT_ROOT,
        RUN_AUTHORIZATION_PATH,
        now_utc=_utc_now(),
    )
    ...
    lease = verify_consumed_execution_lease_v5(PROJECT_ROOT, authorization)

    # 运行时包的实际 hash/argv 验证可触发数值库导入，因此必须放在租约之后。
    from loop167_phase_b.runtime_lock_v5 import validate_runtime_lock_v5

    runtime_lock = require_canonical_json(PROJECT_ROOT / authorization.runtime_lock_binding["path"])
    validate_runtime_lock_v5(
        PROJECT_ROOT,
        runtime_lock,
...
def main() -> int:
    ...
    if args.preflight:
        ...
        print(
            json.dumps(
                {
                    "schema": "axon_loop167_phase_b_static_preflight_receipt_v5",
