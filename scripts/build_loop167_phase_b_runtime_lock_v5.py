#!/usr/bin/env python3
"""Build or verify the v5 lock for the future isolated Phase-B controller."""
...
from loop167_phase_b.runtime_lock_v5 import build_runtime_lock_payload_v5  # noqa: E402

CONTROLLER_PATH = PROJECT_ROOT / CONTROLLER_V5_RELATIVE_PATH
EXECUTION_CONTRACT_PATH = PROJECT_ROOT / EXECUTION_CONTRACT_V5_RELATIVE_PATH
...
def build_payload() -> dict[str, Any]:
    return build_runtime_lock_payload_v5(
        PROJECT_ROOT,
...
def main() -> int:
    ...
    if args.write:
        safe_output = ensure_v5_static_artifact_parent(PROJECT_ROOT, RUNTIME_LOCK_RELATIVE_PATH)
        digest = write_new(safe_output, payload)
    else:
        if not safe_output.is_file() or safe_output.read_bytes() != expected:
            raise SystemExit("Phase-B runtime lock v5 is missing or drifted")
