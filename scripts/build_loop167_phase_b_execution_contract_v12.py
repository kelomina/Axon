#!/usr/bin/env python3
"""Build or verify the immutable Loop167 Phase-B v12 execution contract."""
...
def build_payload(*, parent_v10_prelease_attestation_binding: dict[str, str]) -> dict[str, Any]:
    return build_execution_contract_payload_v12(
        safe_project_root(PROJECT_ROOT),
        parent_v10_prelease_attestation_binding=parent_v10_prelease_attestation_binding,
    )
...
def main() -> int:
    ...
    root = safe_project_root(PROJECT_ROOT)
    attestation_path = safe_project_path(root, PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH, require_exists=False)
    attestation_payload = build_parent_v10_prelease_attestation_payload_v12(root)
    expected_attestation = canonical_json_bytes(attestation_payload)
    if args.write_parent_attestation:
        output_path = ensure_v12_static_artifact_parent(root, PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH)
        digest = _write_new(output_path, attestation_payload)
        print(json.dumps({"path": PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH, "sha256": digest}, sort_keys=True))
        return 0
    if not attestation_path.is_file() or attestation_path.read_bytes() != expected_attestation:
        raise SystemExit("Parent v10 pre-execution attestation is missing or drifted")

    contract_path = safe_project_path(root, EXECUTION_CONTRACT_RELATIVE_PATH, require_exists=False)
    payload = build_payload(parent_v10_prelease_attestation_binding=_binding(PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH))
    expected_contract = canonical_json_bytes(payload)
    if args.write_contract:
        output_path = ensure_v12_static_artifact_parent(root, EXECUTION_CONTRACT_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not contract_path.is_file() or contract_path.read_bytes() != expected_contract:
            raise SystemExit("Phase-B execution contract v12 is missing or drifted")
