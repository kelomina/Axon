#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""目标评估（良性扩充重训后）：零误报白 + 黑召回>99% 在 v2/string 改进版 Stage-2 上。

与 goal_eval_739k.py 同逻辑，仅把输入/输出目录切到 full_739k_benign：
  1) val 阈值下的 FP/FN/recall
  2) 全阈值扫描：能否 FP=0 且 recall>99%
  3) 标签噪声视角：FN 中模型>90%确信良性的数量（疑似误标白）
输出 reports/full_739k_benign/goal_eval.json + test739k_benign_stage2v2_predictions.csv
"""
...
OUT = PROJECT_ROOT / "reports" / "full_739k_benign"
...
def main():
    t0 = time.time()
    print("=== Goal eval (benign): zero-FP + recall>99% (v2/string Stage-2) ===")
    ...
    # 保存 test 预测
    pred_csv = OUT / "test739k_benign_stage2v2_predictions.csv"
    ...
    out = {"val_threshold": thr, "fp": fp0, "fn": fn0, "recall": rec0,
           ...
           "prev_expansion_goal_eval": json.load(
               (PROJECT_ROOT / "reports" / "full_739k" / "goal_eval.json").open(encoding="utf-8"))
           if (PROJECT_ROOT / "reports" / "full_739k" / "goal_eval.json").exists() else None,
