#!/usr/bin/env python3
"""Seal the active v4-only source closure for Loop167 Phase B."""
...
from loop167_phase_b.invocation_v4 import CONTROLLER_V4_RELATIVE_PATH  # noqa: E402
from loop167_phase_b.preflight_v4 import (  # noqa: E402
    EXPECTED_BLOCKERS,
...
SOURCE_CLOSURE_V3_PATH = PROJECT_ROOT / SOURCE_CLOSURE_V3_RELATIVE_PATH
EXECUTION_CONTRACT_PATH = PROJECT_ROOT / EXECUTION_CONTRACT_RELATIVE_PATH
RUNTIME_LOCK_V4_PATH = PROJECT_ROOT / RUNTIME_LOCK_RELATIVE_PATH
CLOSURE_V4_PATH = PROJECT_ROOT / SOURCE_CLOSURE_RELATIVE_PATH
CONTROLLER_PATH = PROJECT_ROOT / CONTROLLER_V4_RELATIVE_PATH

# 只列入当前 v4 运行链及其直接/递归导入；历史 v1-v3 路径只通过 provenance hash 保留。
V4_SOURCE_PATHS = (
    "src/loop167_phase_b/__init__.py",
    "src/loop167_phase_b/contracts.py",
    "src/loop167_phase_b/path_safety_v4.py",
    "src/loop167_phase_b/invocation_v4.py",
    "src/loop167_phase_b/execution_contract_v4.py",
    "src/loop167_phase_b/runtime_lock_v4.py",
    "src/loop167_phase_b/preflight_v4.py",
    "src/loop167_phase_b/resource_guard_v4.py",
    ...
    "scripts/build_loop167_phase_b_execution_contract_v4.py",
    "scripts/build_loop167_phase_b_runtime_lock_v4.py",
    "scripts/build_loop167_phase_b_resource_guard_v4.py",
    "scripts/build_loop167_phase_b_execution_authorization_v4.py",
    "scripts/run_loop167_phase_b_controller_v4.py",
    "scripts/seal_loop167_phase_b_source_closure_v4.py",
    "tests/test_loop167_phase_b_package_import.py",
    "tests/test_loop167_phase_b_v4_controller.py",
    "tests/test_loop167_phase_b_v4_preflight.py",
    "tests/test_loop167_phase_b_v4_source_closure.py",
    "tests/test_loop167_phase_b_v4_execution_contract.py",
    "tests/test_loop167_phase_b_v4_execution_authorization.py",
    "tests/test_loop167_phase_b_lease_v4.py",
    "tests/test_loop167_phase_b_v4_invocation.py",
    "tests/test_loop167_phase_b_v4_path_safety.py",
    "tests/test_loop167_phase_b_v4_runtime_lock.py",
...
def _binding(path: Path) -> dict[str, str]:
    if not path.is_file() or path.is_symlink():
        raise FileNotFoundError(f"Required v4 closure path is missing or unsafe: {path}")
...
def _validate_provenance() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    prior_closure = require_canonical_json(SOURCE_CLOSURE_V3_PATH)
    if prior_closure.get("schema") != "axon_loop167_phase_b_source_closure_v3":
        raise ValueError("Phase-B v3 provenance schema drifted")
    return protocol, phase_a_bindings


def _validate_static_v4_artifacts(protocol_binding: dict[str, str]) -> None:
    verify_execution_contract_v4(
        PROJECT_ROOT,
        _binding(EXECUTION_CONTRACT_PATH),
        expected_protocol_binding=protocol_binding,
    )
    runtime_lock = require_canonical_json(RUNTIME_LOCK_V4_PATH)
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v4":
        raise ValueError("Phase-B runtime lock v4 schema drifted")
    if runtime_lock.get("controller") != _binding(CONTROLLER_PATH):
        raise ValueError("Phase-B runtime lock v4 controller binding drifted")
    if runtime_lock.get("execution_contract") != _binding(EXECUTION_CONTRACT_PATH):
        raise ValueError("Phase-B runtime lock v4 execution-contract binding drifted")


def build_source_closure_v4_payload() -> dict[str, Any]:
    protocol, phase_a_bindings = _validate_provenance()
    protocol_binding = _binding(PROTOCOL_PATH)
    _validate_static_v4_artifacts(protocol_binding)
    source_files = [_binding(PROJECT_ROOT / relative_path) for relative_path in V4_SOURCE_PATHS]
    return {
        "schema": "axon_loop167_phase_b_source_closure_v4",
        "loop_id": "loop167_ember_v3_novel_delta",
        "scope": V4_SCOPE,
        "supersedes_source_closure_v3": _binding(SOURCE_CLOSURE_V3_PATH),
        ...
        "runtime_lock_v4": _binding(RUNTIME_LOCK_V4_PATH),
...
def main() -> int:
    ...
    payload = build_source_closure_v4_payload()
    expected = canonical_json_bytes(payload)
    if args.write:
        digest = write_new(CLOSURE_V4_PATH, payload)
    else:
        if not CLOSURE_V4_PATH.is_file() or CLOSURE_V4_PATH.read_bytes() != expected:
            raise SystemExit("Phase-B source closure v4 is missing or drifted")
        digest = sha256_file(CLOSURE_V4_PATH)
