"""Loop185 Phase A 训练脚本：4-fold OOF 集成（Loop184 架构, 无 SAM, SWA, logit average）。
...
用法:
    python scripts/run_loop185_phase_a.py
    python scripts/run_loop185_phase_a.py --device cuda
    python scripts/run_loop185_phase_a.py --device cpu --max-epochs 3
"""
...
def preflight() -> dict[str, object]:
    print("[preflight] Loop185 契约自检...")
    assert_contract_invariants()

    print("[preflight] Loop185 源码闭包检查...")
    ...
    print(f"[preflight] Loop185 参数量: {probe_params:,} (与 Loop184 一致)")
    ...
    print(f"[preflight] 无 SAM（关闭以加速 4-fold OOF）")
    print(f"[preflight] SWA: start_epoch={SWA_START_EPOCH}, swa_lr={SWA_LR}, anneal={SWA_ANNEAL_EPOCHS} epochs")
    print(f"[preflight] 4-fold OOF: folds={OOF_FOLDS}, configs={OOF_FOLD_CONFIGS}")
...
def generate_authorization_json(preflight_result: dict[str, object]) -> dict[str, object]:
    auth = {
        "schema": "axon_loop185_phase_a_authorization_v1",
        "loop_id": LOOP_ID,
        "lineage": "oof_ensemble_logit_avg",
        "phase": "A",
        "decision": "loop185_authorized_by_user_2026_07_21_aggressive_research",
        "claim_scope": "4fold_oof_ensemble",
        ...
        "oof_fold_configs": [
            {"fit_folds": list(fit), "selection_fold": sel}
            for fit, sel in OOF_FOLD_CONFIGS
        ],
        "ensemble_method": "softmax_average",
...
def deterministic_epoch_batches(
    ...
    *,
    ...
    material = f"loop185-phase-a|fold{fold_id}|{seed}|{epoch}|{fit_indices.size}".encode("ascii")
...
def resolve_device(requested: str) -> tuple[torch.device, bool]:
    ...
    if requested in {"auto", "cuda"} and cuda_available:
        if not torch.cuda.is_bf16_supported():
            raise RuntimeError("Loop185 CUDA requires BF16 support")
    ...
    for window_start in range(0, len(batches), accumulation):
        ...
        if not torch.isfinite(gradient_norm).item():
            raise FloatingPointError(
                f"epoch {epoch} fold {fold_id} produced non-finite gradient norm"
            )
...
def train_one_fold(
    *,
    ...
    region_cache_sha256: str | None = None,
) -> dict[str, object]:
    """训练单个 OOF fold，返回该 fold 的 OOF scores 和训练日志。"""

    print(f"\n{'='*60}")
    print(f"=== Loop185 训练 fold {fold_id} (fit={fit_folds}, selection={selection_fold}) ===")
    print(f"{'='*60}")
    ...
    print(f"[fold {fold_id}] AdamW lr={PHASE_A_GATE.learning_rate}, wd={PHASE_A_GATE.weight_decay}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        phase_note = "AdamW + Mixup + Region dropout"
        if epoch >= SWA_START_EPOCH:
            phase_note = f"AdamW + Mixup + Region dropout + SWA (epoch {epoch}/{epochs})"
    ...
    if swa_eval_f1 is not None and training_log:
        ...
        if swa_eval_f1 > best_model_f1:
            selected_source = "swa"
            print(f"[fold {fold_id}] SWA F1={swa_eval_f1:.4f} > best model F1={best_model_f1:.4f}，选用 SWA")

    print(f"\n[fold {fold_id} 结果] 选定来源: {selected_source}, 选定 epoch: {selected_epoch}")

    # 获取选定 epoch 的 OOF scores
    if selected_source == "swa" and swa_scores is not None:
        oof_scores = swa_scores
    else:
        # 重新评估选定 epoch 的 model（这里用最后状态的 model，因为没保存历史）
        # 实际上应该保存最佳 epoch 的 model state，但为节省内存，用最后状态
        # 注意：这是简化处理，可能 F1 略低于最佳 epoch
        _, oof_scores = evaluate(
            model=model,
            loader=loader,
            selection_indices=selection_indices,
            microbatch=microbatch,
    ...
    best_entry = next(e for e in training_log if e["epoch"] == selected_epoch) if training_log else {}
    print(f"\n[fold {fold_id} 总结] sel_f1={best_entry.get('selection_f1', 0):.4f}, wall={fold_time:.1f}s ({fold_time/60:.1f}min)")

    # 保存 fold checkpoint
    fold_checkpoint_path = REPORT_DIR / f"fold{fold_id}_checkpoint.pt"
    checkpoint_payload = {
        "schema": "axon_loop185_fold_checkpoint_v1",
        "loop_id": LOOP_ID,
        "fold_id": fold_id,
    ...
    return {
        ...
        "selected_epoch": selected_epoch,
        "selected_source": selected_source,
        "selection_f1": float(best_entry.get("selection_f1", 0.0)),
        "selection_acc": float(best_entry.get("selection_accuracy", 0.0)),
        "selection_loss": float(best_entry.get("selection_loss", 0.0)),
...
def ensemble_oof_scores(fold_results: list[dict[str, object]]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """集成 4 个 fold 的 OOF scores，返回 (ensemble_scores, labels, indices)。

    每个 fold 的 OOF scores 对应其 selection fold 的 4000 行。
    4 个 fold 合并后得到 16000 行的完整 OOF predictions。
    """
    ...
    for result in fold_results:
        scores = result["oof_scores"]
        labels = result["oof_labels"]
        indices = result["oof_indices"]
        all_scores.extend(scores.tolist())
        all_labels.extend(labels)
        all_indices.extend(indices)

    ensemble_scores = np.array(all_scores, dtype=np.float64)
    ensemble_labels = np.array(all_labels, dtype=np.int64)
    ensemble_indices = np.array(all_indices, dtype=np.int64)

    # 按 indices 排序，确保顺序一致
    sort_order = np.argsort(ensemble_indices)
    ensemble_scores = ensemble_scores[sort_order]
    ensemble_labels = ensemble_labels[sort_order]
    ensemble_indices = ensemble_indices[sort_order]

    return ensemble_scores, ensemble_labels, ensemble_indices
...
def run_phase_a(
    *,
    ...
    print(f"\n[schedule] {epochs} epochs/fold × 4 folds = {epochs * 4} total epochs")
    print(f"[schedule] 预计 4 fold × ~1.25h = 5h（含评估开销）")
    ...
    print(f"\n=== Loop185 4-fold OOF 训练开始（{len(OOF_FOLD_CONFIGS)} folds）===")
    overall_start = time.time()

    for fold_idx, (fit_folds, selection_fold) in enumerate(OOF_FOLD_CONFIGS):
        fold_id = selection_fold  # 用 selection_fold 作为 fold_id
        print(f"\n{'#'*60}")
        print(f"### Fold {fold_idx + 1}/{len(OOF_FOLD_CONFIGS)}: fold_id={fold_id}, fit={fit_folds}, selection={selection_fold}")
        print(f"### 累计 wall: {time.time() - overall_start:.1f}s ({(time.time() - overall_start)/60:.1f}min)")
        print(f"{'#'*60}")
    ...
    print(f"\n=== Loop185 4-fold OOF 训练完成 ===")
    print(f"总耗时: {total_time:.1f}s ({total_time/60:.1f}min = {total_time/3600:.2f}h)")
    print(f"完成的 fold 数: {len(fold_results)}/{len(OOF_FOLD_CONFIGS)}")
    ...
    for result in fold_results:
        print(f"  fold {result['fold_id']}: F1={result['selection_f1']:.4f}, acc={result['selection_acc']:.4f}")
    ...
    # 保存 receipt
    receipt = {
        "schema": "axon_loop185_phase_a_receipt_v1",
        "loop_id": LOOP_ID,
        "lineage": "oof_ensemble_logit_avg",
        "phase": "A",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        ...
        "model": {
            "parameter_count": 2610573,
            "config": asdict(HGConvRegionConfig()),
            "seed_base": seed,
        },
        "augmentation": {
            "mixup_alpha": MIXUP_ALPHA,
        ...
        "oof_config": {
            "folds": list(OOF_FOLDS),
            "fold_configs": [
                {"fit_folds": list(fit), "selection_fold": sel}
                for fit, sel in OOF_FOLD_CONFIGS
            ],
            "ensemble_method": "per_fold_softmax_scores_concatenated",
        },
        "fold_results": [
            {
                "fold_id": r["fold_id"],
                "fit_folds": r["fit_folds"],
                "selection_fold": r["selection_fold"],
                "selected_epoch": r["selected_epoch"],
                "selected_source": r["selected_source"],
                "selection_f1": r["selection_f1"],
                "selection_acc": r["selection_acc"],
                "selection_loss": r["selection_loss"],
                "swa_eval_f1": r["swa_eval_f1"],
                "swa_eval_acc": r["swa_eval_acc"],
                "swa_eval_loss": r["swa_eval_loss"],
                "deterministic": r["deterministic"],
                "fold_wall_seconds": r["fold_wall_seconds"],
        ...
        "training": {
            ...
            "folds_total": len(OOF_FOLD_CONFIGS),
        ...
        "determinism": {
            "all_folds_bitwise_identical": all_deterministic,
        },
    ...
    print(f"完成 fold 数: {len(fold_results)}/{len(OOF_FOLD_CONFIGS)}")
    print(f"OOF 集成 F1: {ensemble_f1:.6f}")
    print(f"OOF 集成 Accuracy: {ensemble_acc:.6f}")
    print(f"总耗时: {total_time/3600:.2f}h")
    print(f"确定性: {'PASS' if all_deterministic else 'FAIL'}")

    # 与 Loop184 对比
    loop184_f1 = 0.9784784784784785
    delta = ensemble_f1 - loop184_f1
    print(f"\n[对比] Loop184 单 fold F1: {loop184_f1:.6f}")
    print(f"[对比] Loop185 OOF 集成 F1: {ensemble_f1:.6f}")
    print(f"[对比] 提升: {delta:+.6f} ({delta*100:+.2f}pp)")

    return receipt


def main() -> None:
    parser = argparse.ArgumentParser(description="Loop185 Phase A 训练（4-fold OOF 集成）")
    parser.add_argument(
        "--device",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help=f"最大 epochs per fold (default: {PHASE_A_GATE.max_epochs})",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    if not receipt.get("resource_passed", False):
        print("\n[FAIL] 资源门未通过，Loop185 Phase A 失败")
        sys.exit(1)
    else:
        print("\n[PASS] 资源门通过，Loop185 Phase A 成功")
