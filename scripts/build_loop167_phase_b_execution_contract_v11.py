#!/usr/bin/env python3
"""Build or verify the immutable Loop167 Phase-B v11 execution contract."""
...
def build_payload(*, parent_v10_prelease_attestation_binding: dict[str, str]) -> dict[str, Any]:
    return build_execution_contract_payload_v11(
...
def main() -> int:
    ...
    attestation_payload = build_parent_v10_prelease_attestation_payload_v11(root)
    expected_attestation = canonical_json_bytes(attestation_payload)
    if args.write_parent_attestation:
        output_path = ensure_v11_static_artifact_parent(root, PARENT_V10_PRELEASE_ATTESTATION_RELATIVE_PATH)
    ...
    if args.write_contract:
        output_path = ensure_v11_static_artifact_parent(root, EXECUTION_CONTRACT_RELATIVE_PATH)
        digest = _write_new(output_path, payload)
    else:
        if not contract_path.is_file() or contract_path.read_bytes() != expected_contract:
            raise SystemExit("Phase-B execution contract v11 is missing or drifted")
