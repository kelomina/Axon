#!/usr/bin/env python3
"""Run the contained Loop167 Phase-B v9 controller route."""
...
from loop167_phase_b.child_attestation_v9 import (  # noqa: E402
    build_child_job_attestation_payload_v9,
    write_child_job_attestation_v9,
)
...
from loop167_phase_b.execution_authorization_v9 import (  # noqa: E402
...
from loop167_phase_b.execution_contract_v9 import (  # noqa: E402
...
from loop167_phase_b.invocation_v9 import (  # noqa: E402
    bootstrap_thread_environment_v9,
    validate_current_runtime_invocation_v9,
)
from loop167_phase_b.lease_v9 import (  # noqa: E402
    consume_execution_lease_v9,
    verify_consumed_execution_lease_v9,
)
from loop167_phase_b.path_safety_v4 import safe_project_path  # noqa: E402
from loop167_phase_b.preflight_v9 import validate_static_preflight_v9  # noqa: E402
from loop167_phase_b.supervisor_v9 import _write_new_json  # noqa: E402

RECEIPT_SCHEMA = "axon_loop167_phase_b_execution_receipt_v9"
...
def _resolve_fixed_raw_root_adapter(root: Path, raw_root_relative: str) -> Path:
    ...
    if raw_root_relative != RAW_ROOT_RELATIVE_PATH:
        raise PhaseBContractError("v9 execution contract raw root drifted")
    ...
    for component in raw_root_relative.split("/"):
        ...
        try:
            ...
            raise PhaseBContractError("v9 fixed raw root is unavailable before lease consumption") from error
        if _is_link_or_reparse(stat_result) or not stat.S_ISDIR(stat_result.st_mode):
            raise PhaseBContractError("v9 fixed raw root traverses an unsafe directory")
    if candidate != cursor:
        raise PhaseBContractError("v9 fixed raw root resolution drifted")
    return candidate
...
def _ensure_output_parent_after_lease(root: Path, output_path: Path) -> None:
    try:
        ...
        raise PhaseBContractError("v9 sealed output path escapes the project root") from error
    cursor = root
    for component in relative_parent.parts:
        ...
        try:
            ...
            raise PhaseBContractError("v9 sealed output parent cannot be prepared") from error
        if _is_link_or_reparse(stat_result) or not stat.S_ISDIR(stat_result.st_mode):
            raise PhaseBContractError("v9 sealed output parent is unsafe")
...
def _assert_authorization_matches_static_preflight(authorization: Any, static_receipt: Any) -> None:
    ...
    for name, expected_binding in expected.items():
        if dict(getattr(authorization, name)) != dict(expected_binding):
            raise PhaseBContractError(f"v9 execution authorization {name} drifted from static preflight")
...
def _assert_static_preflight_is_unchanged(initial: Any, refreshed: Any) -> None:
    ...
    if any(getattr(initial, field) != getattr(refreshed, field) for field in fields):
        raise PhaseBContractError("v9 static source closure drifted after lease verification")


def _require_elapsed(started_at: float, maximum_seconds: int, *, phase: str) -> None:
    if time.monotonic() - started_at > maximum_seconds:
        raise PhaseBContractError(f"v9 Phase-B {phase} wall-clock budget was exceeded")
...
def _execute_after_lease(
    *,
    ...
    """Import the numerical/raw data plane only after a verified v9 lease exists."""
    ...
    if not required_output_names.issubset(outputs):
        raise PhaseBContractError("v9 execution authorization output catalog drifted")
    ...
    if not isinstance(raw_context_contract, dict):
        raise PhaseBContractError("v9 Phase-B raw-context contract is unavailable")
    ...
    if not isinstance(maximum_source_file_bytes, int) or not isinstance(reader_chunk_bytes, int):
        raise PhaseBContractError("v9 Phase-B raw-context limits drifted")
    ...
    if not raw_validation.complete or raw_validation.final_record_sha256 != raw_outcome.raw_ledger_final_record_sha256:
        raise PhaseBContractError("v9 raw progress ledger did not close the sealed one-pass scan")
    ...
    if sampling_contract.get("receipt_key") != "sampling_audit":
        raise PhaseBContractError("v9 execution contract B1 sampling-audit binding drifted")
    if len(cache_receipt.sampling_audit.indicator_counts) != sampling_contract.get("dimension"):
        raise PhaseBContractError("v9 B1 sampling-audit dimension drifted from the execution contract")
    _require_elapsed(
    ...
    if not fit_validation.complete or fit_validation.final_record_sha256 != fit_result.fit_ledger_final_record_sha256:
        raise PhaseBContractError("v9 fit progress ledger did not close all fixed units")
    _require_elapsed(
        fitting_started_at,
...
def _static_preflight(mode: str, *, phase: str) -> Any:
    bootstrap_thread_environment_v9()
    validate_current_runtime_invocation_v9(PROJECT_ROOT, role="controller", mode=mode)
    return validate_static_preflight_v9(
        PROJECT_ROOT,
...
def _launch_id() -> str:
    launch_id = os.environ.get("AXON_LOOP167_V9_LAUNCH_ID", "")
    if len(launch_id) != 64 or any(character not in "0123456789abcdef" for character in launch_id):
        raise PhaseBContractError("v9 controller launch id is unavailable")
    return launch_id
...
def run_execute() -> dict[str, Any]:
    bootstrap_thread_environment_v9()
    validate_current_runtime_invocation_v9(PROJECT_ROOT, role="controller", mode="execute")
    launch_id = _launch_id()
    expected_bindings = _sealed_static_bindings()
    launch_receipt_path = resolve_output_catalog_v9(PROJECT_ROOT, FIXED_OUTPUT_CATALOG)[
        "supervisor_launch_receipt"
    ]
    child_attestation = build_child_job_attestation_payload_v9(
        PROJECT_ROOT,
        launch_receipt_path=launch_receipt_path,
        launch_id=launch_id,
        expected_bindings=expected_bindings,
    )
    write_child_job_attestation_v9(PROJECT_ROOT, child_attestation)
    static_receipt = validate_static_preflight_v9(
        PROJECT_ROOT,
    ...
    authorization = validate_execution_authorization_v9(
        PROJECT_ROOT,
    ...
    contract = verify_execution_contract_v9(PROJECT_ROOT, authorization.execution_contract_binding)
    consumed_lease = consume_execution_lease_v9(
        PROJECT_ROOT,
        PROJECT_ROOT / RUN_AUTHORIZATION_RELATIVE_PATH,
        now_utc=_utc_now(),
        launch_id=launch_id,
    ...
    if consumed_lease.authorization_sha256 != authorization.authorization_sha256:
        raise PhaseBContractError("v9 consumed lease authorization drifted")
    lease = verify_consumed_execution_lease_v9(
        PROJECT_ROOT,
        authorization,
        launch_id=launch_id,
        now_utc=_utc_now(),
    )
    post_lease_static_receipt = validate_static_preflight_v9(
        PROJECT_ROOT,
        source_closure_binding=expected_bindings["source_closure"],
        controller_binding=expected_bindings["controller"],
        supervisor_binding=expected_bindings["supervisor"],
...
def main() -> int:
    ...
    if args.preflight:
        ...
        print(
            json.dumps(
                {
                    "schema": "axon_loop167_phase_b_static_preflight_receipt_v9",
