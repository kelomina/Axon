"""Loop182 Phase A 训练脚本：Multi-Scale HGConv 架构改进。

Loop182 = Loop179 基础架构 + MultiScaleHGConvBlock（并行 filter_lengths=[8,16,32,64]）。
训练超参与 Loop179 完全一致，以隔离多尺度卷积的因果效应：
...
用法:
    python scripts/run_loop182_phase_a.py
    python scripts/run_loop182_phase_a.py --device cuda
    python scripts/run_loop182_phase_a.py --device cpu --max-epochs 3
"""
...
from src.loop182.contracts import LOOP_ID


PROJECT_ROOT = Path(__file__).resolve().parents[1]
REPORT_DIR = PROJECT_ROOT / "reports" / "roadmap_9997" / "loop182"


def preflight() -> dict[str, object]:
    print("[preflight] Loop182 契约自检...")
    assert_contract_invariants()

    print("[preflight] Loop182 源码闭包检查...")
    ...
    print(f"[preflight] Loop182 参数量: {probe_params:,}")
...
def generate_authorization_json(preflight_result: dict[str, object]) -> dict[str, object]:
    auth = {
        "schema": "axon_loop182_phase_a_authorization_v1",
        "loop_id": LOOP_ID,
        "lineage": "multi_scale_hgconv_region",
        "phase": "A",
        "decision": "loop182_authorized_by_user_2026_07_20_aggressive_research",
        "claim_scope": "multi_scale_hgconv_architecture_improvement",
...
def deterministic_epoch_batches(
    ...
    *,
    ...
    material = f"loop182-phase-a|{seed}|{epoch}|{fit_indices.size}".encode("ascii")
...
def resolve_device(requested: str) -> tuple[torch.device, bool]:
    ...
    if requested in {"auto", "cuda"} and cuda_available:
        if not torch.cuda.is_bf16_supported():
            raise RuntimeError("Loop182 CUDA requires BF16 support")
...
def run_phase_a(
    *,
    ...
    print(f"[model] 参数量: {param_count:,} (vs Loop179 1,604,677, +{(param_count - 1604677) / 1604677 * 100:.2f}%)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        sel_f1 = 2 * tp / (2 * tp + fp + fn) if (2 * tp + fp + fn) > 0 else 0.0
    ...
    print(f"\n=== Loop182 资源门: {'PASS' if resource_passed else 'FAIL'} ===")
    ...
    if resource_passed:
        torch.save(
            {
                "schema": "axon_loop182_phase_a_checkpoint_v1",
                "loop_id": LOOP_ID,
    ...
    receipt = {
        "schema": "axon_loop182_phase_a_receipt_v1",
        "loop_id": LOOP_ID,
        "lineage": "multi_scale_hgconv_region",
        "phase": "A",
        "generated_at": datetime.now(timezone.utc).isoformat(),
    ...
    print(f"选定 epoch: {selected_epoch}, best_epoch: {selected_epoch}")
    if training_log:
    ...
    print(f"参数量: {param_count:,} (vs Loop179 1,604,677, +{(param_count - 1604677) / 1604677 * 100:.2f}%)")

    return receipt


def main() -> None:
    parser = argparse.ArgumentParser(description="Loop182 Phase A 训练（Multi-Scale HGConv）")
    parser.add_argument(
        "--device",
    ...
    if not receipt["resource_passed"]:
        print("\n[FAIL] 资源门未通过，Loop182 Phase A 失败")
        sys.exit(1)
    else:
        print("\n[PASS] 资源门通过，Loop182 Phase A 成功")
        if receipt.get("training", {}).get("selection_losses"):
            best_f1 = max(e.get("selection_f1", 0.0) for e in [])
            # 从 training_log 读取最佳 F1
            log_path = receipt.get("training_log_path")
            if log_path:
                with open(log_path, "r", encoding="utf-8") as f:
                    for line in f:
                        entry = json.loads(line)
                        if entry.get("epoch") == receipt["training"]["selected_epoch"]:
                            print(f"最佳 selection F1: {entry.get('selection_f1', 0.0):.4f}")
                            break
