#!/usr/bin/env python3
"""Seal the complete v7 source closure for the Loop167 Phase-B execution route."""
...
from loop167_phase_b.preflight_v7 import (  # noqa: E402
    EXPECTED_BLOCKERS,
...
V7_SOURCE_PATHS = (
    ...
    "scripts/build_loop167_phase_b_execution_contract_v7.py",
    "scripts/build_loop167_phase_b_runtime_lock_v7.py",
    "scripts/build_loop167_phase_b_resource_guard_v7.py",
    "scripts/build_loop167_phase_b_execution_authorization_v7.py",
    "scripts/run_loop167_phase_b_supervisor_v7.py",
    "scripts/run_loop167_phase_b_controller_v7.py",
    "scripts/probe_loop167_phase_b_v7_suspended_child.py",
    "scripts/seal_loop167_phase_b_source_closure_v7.py",
    "tests/test_loop167_phase_b_package_import.py",
    "tests/test_loop167_phase_b_v7_windows_job.py",
    "tests/test_loop167_phase_b_v7_supervisor.py",
    "tests/test_loop167_phase_b_v7_child_attestation.py",
    "tests/test_loop167_phase_b_v7_loop166_bridge.py",
    "tests/test_loop167_phase_b_v7_path_safety.py",
    "tests/test_loop167_phase_b_v7_resource_guard.py",
    "tests/test_loop167_phase_b_v7_execution_authorization.py",
    "tests/test_loop167_phase_b_lease_v7.py",
    "tests/test_loop167_phase_b_v7_controller.py",
    "tests/test_loop167_phase_b_v7_source_closure.py",
    "tests/test_loop167_phase_b_v7_state_machine.py",
    "tests/test_loop167_phase_b_v7_windows_integration.py",
...
def _validate_static_v7_artifacts() -> None:
    parent_attestation = _binding(PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH)
    verify_parent_v6_prelease_attestation_v7(PROJECT_ROOT, parent_attestation)
    contract_binding = _binding(EXECUTION_CONTRACT_RELATIVE_PATH)
    verify_execution_contract_v7(PROJECT_ROOT, contract_binding)
    ...
    if runtime_lock.get("schema") != "axon_loop167_phase_b_runtime_lock_v7" or runtime_lock.get("loop_id") != LOOP_ID:
        raise ValueError("Phase-B runtime lock v7 schema drifted")
    if runtime_lock.get("controller") != _binding(CONTROLLER_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v7 controller binding drifted")
    if runtime_lock.get("supervisor") != _binding(SUPERVISOR_RELATIVE_PATH):
        raise ValueError("Phase-B runtime lock v7 supervisor binding drifted")
    if runtime_lock.get("execution_contract") != contract_binding:
        raise ValueError("Phase-B runtime lock v7 execution-contract binding drifted")


def build_source_closure_v7_payload() -> dict[str, Any]:
    _validate_static_v7_artifacts()
    if len(V7_SOURCE_PATHS) != len(set(V7_SOURCE_PATHS)):
        raise ValueError("v7 source closure repeats a path")
    if not REQUIRED_SOURCE_PATHS.issubset(set(V7_SOURCE_PATHS)):
        raise ValueError("v7 source closure omits a required control-plane path")
    return {
        "schema": SOURCE_CLOSURE_SCHEMA,
        "loop_id": LOOP_ID,
        "scope": V7_SCOPE,
        "parent_v6_prelease_attestation": _binding(PARENT_V6_PRELEASE_ATTESTATION_RELATIVE_PATH),
        ...
        "source_files": [_binding(relative_path) for relative_path in V7_SOURCE_PATHS],
