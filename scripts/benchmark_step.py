#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""训练步 A/B benchmark：batch 64 完整 forward+backward，wall 计时 + loss 一致性。

在 GPU 独占时运行（先停掉正在跑的训练），对比优化前后：
  python scripts/benchmark_step.py            # 优化后（当前代码）
  git stash push -- src/dsra/mhdsra2/improved_dsra_mha.py
  python scripts/benchmark_step.py --tag base  # 优化前基线
  git stash pop

输出: reports/benchmark_step_<tag>.json + 打印 mean/p50 step 时间与 loss
"""
from __future__ import annotations

import os
import sys
import time
from pathlib import Path

os.environ.setdefault("OMP_NUM_THREADS", "1")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")

PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT / "src"))

import json  # noqa: E402

import numpy as np  # noqa: E402
import torch  # noqa: E402

from config import AxonExperimentConfig  # noqa: E402
from dataset import FeatureCacheDataset, SubDataset  # noqa: E402
from model import AxonMalwareModel  # noqa: E402

CKPT = PROJECT_ROOT / "models" / "full_739k_benign" / "best_model_739k.pt"
TRUNCATE_BYTE_LENGTH = 4096
BATCH_SIZE = 64
N_STEPS = 30
N_WARMUP = 3


class _TruncatedByteDataset(torch.utils.data.Dataset):
    def __init__(self, base, max_len: int = TRUNCATE_BYTE_LENGTH):
        self.base = base
        self.max_len = int(max_len)

    def __len__(self):
        return len(self.base)

    def __getitem__(self, idx: int):
        item = self.base[idx]
        byte_seq = item[0]
        if byte_seq.shape[0] > self.max_len:
            byte_seq = byte_seq[: self.max_len]
        return (byte_seq,) + tuple(item[1:])


def main() -> None:
    tag = "opt"
    for a in sys.argv[1:]:
        if a.startswith("--tag="):
            tag = a.split("=", 1)[1]

    device = torch.device("cuda")
    print(f"[bench] tag={tag} batch={BATCH_SIZE} n={N_STEPS} device={device}")

    ckpt = torch.load(CKPT, map_location="cpu", weights_only=False)
    raw_cfg = ckpt["config"]
    config = AxonExperimentConfig.from_dict(raw_cfg) if isinstance(raw_cfg, dict) else raw_cfg

    dataset = FeatureCacheDataset(
        data_dir=str(PROJECT_ROOT / "data"),
        cache_dir=str(PROJECT_ROOT / "data" / ".cache"),
        max_byte_length=config.max_byte_length,
        pe_feature_dim=config.pe_feature_dim,
        stat_feature_dim=config.stat_feature_dim,
        max_samples_per_class=None,
        axon_config=config,
    )
    n_sub = BATCH_SIZE * 4  # 4 个 batch 轮换，避免数据 cache 差异
    sub = SubDataset(_TruncatedByteDataset(dataset, TRUNCATE_BYTE_LENGTH), list(range(n_sub)))
    loader = torch.utils.data.DataLoader(sub, batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
    batches = [tuple(t.to(device) for t in b) for b in loader]

    model = AxonMalwareModel(config)
    model.load_state_dict(ckpt["model_state_dict"])
    model.to(device).train()
    opt = torch.optim.AdamW(model.parameters(), lr=3e-5)
    criterion = torch.nn.CrossEntropyLoss()

    def step(batch):
        byte_seq, pe, stat, label = batch
        out = model(byte_seq, pe, stat)
        loss = criterion(out["logits"], label)
        loss.backward()
        opt.step()
        opt.zero_grad()
        return float(loss.detach().item())

    for i in range(N_WARMUP):
        step(batches[i % len(batches)])
    torch.cuda.synchronize()
    print("[warmup] done")

    times = []
    losses = []
    for i in range(N_STEPS):
        t0 = time.perf_counter()
        loss = step(batches[i % len(batches)])
        torch.cuda.synchronize()
        times.append(time.perf_counter() - t0)
        losses.append(loss)

    a = np.array(times)
    print(f"[result] tag={tag} mean={a.mean():.4f}s  p50={np.median(a):.4f}  "
          f"p90={np.percentile(a, 90):.4f}  min={a.min():.4f}  "
          f"loss_first={losses[0]:.4f} loss_last={losses[-1]:.4f}")
    print(f"[estimate] epoch(569170/{BATCH_SIZE} steps) = {a.mean()*569170/BATCH_SIZE/3600:.2f} h")

    out = {
        "tag": tag, "batch_size": BATCH_SIZE, "n_steps": N_STEPS,
        "mean_sec": float(a.mean()), "p50_sec": float(np.median(a)),
        "p90_sec": float(np.percentile(a, 90)), "min_sec": float(a.min()),
        "loss_first": losses[0], "loss_last": losses[-1], "losses": losses,
    }
    out_path = PROJECT_ROOT / "reports" / f"benchmark_step_{tag}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(out, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"[saved] {out_path}")


if __name__ == "__main__":
    main()
